#!/usr/bin/env python3
"""
Model 2l - Pure 1D TCN: Non-causal residual TCN on mel spectrogram time axis
- Input: mel spectrogram (64×300) treated as 300-step sequence of 64-dim features
- Non-causal padding='same': full temporal context (offline 3s clip inference)
- Residual connections: clean residual stream, no skip aggregation
- LayerNorm: stable training without batch statistics
- 8 dilated blocks, cycling dilations [1,2,4,8,16,32,1,2]
- Hypothesis: non-causal receptive field + residuals > causal no-residual (2c)
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 2l - Pure 1D TCN)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Pure 1D TCN (non-causal, residual, LayerNorm)\n")
            f.write(f"  TCN Channels:           {config['tcn_channels']}\n")
            f.write(f"  Dropout Rate:           {config['dropout']}\n")
            f.write(f"  Input Shape:            {config['input_shape']}\n")
            f.write(f"  TCN Blocks:             8 (dilations: 1,2,4,8,16,32,1,2)\n")
            f.write(f"  Padding:                same (non-causal — full temporal context)\n")
            f.write(f"  Activation:             ReLU6 (quantization-friendly)\n")
            f.write(f"  Residual Connections:   Yes (clean residual stream)\n")
            f.write(f"  Normalization:          LayerNorm (pre-activation)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  ✓ Pure 1D TCN (non-causal residual)\n")
            f.write(f"  - Non-causal dilated convs: padding=same, dilations=[1,2,4,8,16,32,1,2]\n")
            f.write(f"  - Clean residual stream (no skip aggregation)\n")
            f.write(f"  - Pre-norm LayerNorm + ReLU6 (quantization-friendly)\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    """Pure 1D TCN: non-causal residual TCN on the time axis of the mel spectrogram.

    Key differences from causal TCNs (2a-2k):
    - padding='same' (non-causal): each frame sees the full 300-frame context
    - Residual connections: clean residual stream without skip aggregation
    - LayerNorm (pre-norm): stable training, no batch statistics
    - 8 blocks, cycling dilations [1,2,4,8,16,32,1,2]

    Receptive field (padding='same', kernel=3):
      Each dilated conv covers ±(kernel-1)*d = ±2d frames.
      Total: 2*(1+2+4+8+16+32+1+2) = 132 frames each side → full 300-frame coverage.
    """
    inputs = layers.Input(shape=input_shape)           # (n_mels, time, 1)
    # Reshape to time-first: (time, n_mels)
    x = layers.Permute((2, 1, 3))(inputs)
    x = layers.Reshape((input_shape[1], input_shape[0]))(x)

    # Linear input projection (no activation — LayerNorm follows)
    x = layers.Conv1D(channels, 1, use_bias=False, name='input_proj')(x)

    # 8 residual blocks with cycling dilations
    for i, d in enumerate([1, 2, 4, 8, 16, 32, 1, 2]):
        res = x
        x = layers.LayerNormalization(name=f'ln_{i}')(x)
        x = layers.Conv1D(channels, 3, dilation_rate=d, padding='same',
                          activation='relu6', name=f'dconv_{i}')(x)
        x = layers.Dropout(dropout, name=f'drop_{i}a')(x)
        x = layers.Conv1D(channels, 1, name=f'pw_{i}')(x)   # pointwise mix
        x = layers.Add(name=f'res_{i}')([x, res])

    x = layers.LayerNormalization(name='ln_final')(x)
    x = layers.GlobalAveragePooling1D()(x)
    x = layers.Dense(256, activation='relu6')(x)
    x = layers.Dropout(dropout)(x)
    outputs = layers.Dense(num_classes, activation='softmax')(x)

    return keras.Model(inputs, outputs, name="MynaNet_Pure1D_TCN")
...
def main():
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING PURE 1D TCN MODEL (non-causal, residual, LayerNorm)")
    print(f"{'=' * 70}")
