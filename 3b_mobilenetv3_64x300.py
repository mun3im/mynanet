#!/usr/bin/env python3
"""
Ablation Study 2b: MobileNetV3Small for Mygardenbird Classification
...
for spectrograms. We adapt the architecture to work directly with 64×300 input
without resizing, preserving temporal and frequency resolution.

Architecture: MobileNetV3Small adapted for 64×300
- Remove first stride-2 conv (preserve resolution)
- Adjust spatial reduction to account for 64×300 shape
...
Version 2c: Based on 7d pipeline with MobileNetV3Small
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MobileNetV3Small (Ablation 2c)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"2b_mobilenetv3,{config['dropout']},{config['augmentation_mode']},"
...
def create_mobilenetv3_small_64x300(num_classes, input_shape, dropout=0.2):
    """
    MobileNetV3Small adapted for 64×300 spectrogram input (NATIVE, NO RESIZE).
    ...
    return keras.Model(inputs, outputs, name="MobileNetV3Small_64x300")
...
def main():
    ...
    print(f"\nAblation 2c: MobileNetV3Small @ 64×300 (NATIVE)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING MOBILENETV3SMALL MODEL (64×300 NATIVE)")
    print(f"{'=' * 70}")
    model = create_mobilenetv3_small_64x300(num_classes, config['input_shape'], config['dropout'])
    model.summary()
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   2b_mobilenetv3 (64×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
