#!/usr/bin/env python3
"""
Depthwise Separable CNN - Ablation Study (Model 2a)
...
Model Architecture: Depthwise Separable CNN (~80K parameters)
  Conv2D(32, 3×3)
  DepthwiseConv2D(3×3) → Conv2D(64, 1×1) → MaxPool2D(2×2)
  DepthwiseConv2D(3×3) → Conv2D(128, 1×1) → MaxPool2D(2×2)
  DepthwiseConv2D(3×3) → Conv2D(256, 1×1) → MaxPool2D(2×2)
  GlobalAveragePooling2D
  Dense(128) → Dropout → Dense(10)

Purpose: Efficient depthwise separable convolutions for minimal parameters
Inherits all features from 7d: SpecAugment, Mixup, fixed split, WIN_LENGTH
"""
...
def get_config():
    ...
    output_dir_name = (
        f"results/2a_depthwise_cnn_64x300_ptq_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Version 7d)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Depthwise Separable CNN (Ablation Study 2b)\n")
            f.write(f"  Architecture:           Conv32 → DepthSep64→128→256 + MaxPool\n")
            f.write(f"  Dropout Rate:           {config['dropout']}\n")
            f.write(f"  Input Shape:            {config['input_shape']}\n")
            f.write(f"  Conv Blocks:            3 depthwise separable blocks (64, 128, 256 filters)\n")
            f.write(f"  Depthwise Conv:         3×3 (per-channel spatial filtering)\n")
            f.write(f"  Pointwise Conv:         1×1 (cross-channel mixing)\n")
            ...
            f.write(f"  Efficiency:             ~9x fewer params than standard Conv2D\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Config: model2b_drp{int(config['dropout'] * 10)}_"
            ...
            f.write(f"2a_depthwise_cnn,{config['dropout']},{config['augmentation_mode']},"
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  ✓ Depthwise Separable CNN (Ablation Study 2b)\n")
            f.write(f"  - 3 depthwise separable blocks: 64→128→256 filters\n")
            f.write(f"  - DepthwiseConv2D (3×3) + Conv2D (1×1) + MaxPool2D\n")
            f.write(f"  - GlobalAveragePooling2D + Dense(128) classifier\n")
            f.write(f"  - ReLU6 activation (quantization-friendly)\n")
            f.write(f"  - ~9x parameter efficiency vs standard Conv2D\n")
...
# --------------------------------------------------------------
# MOBILENET-STYLE CNN MODEL (Ablation Study 2b)
# --------------------------------------------------------------
def create_mobilenet_cnn(num_classes, input_shape, dropout=0.2):
    """
    Create Depthwise Separable CNN model for mygardenbird classification (Ablation Study 2b).

    Architecture (Depthwise Separable Convolutions):
      Input: (64, 300, 1) mel-spectrogram
      Conv2D(32, 3×3, relu6) [standard conv for input]
      DepthwiseConv2D(3×3) → Conv2D(64, 1×1, relu6) → MaxPool2D(2×2) → Dropout
      DepthwiseConv2D(3×3) → Conv2D(128, 1×1, relu6) → MaxPool2D(2×2) → Dropout
      DepthwiseConv2D(3×3) → Conv2D(256, 1×1, relu6) → MaxPool2D(2×2) → Dropout
      GlobalAveragePooling2D
      Dense(128, relu6) → Dropout → Dense(10, softmax)

    Parameters: ~80K (smallest model in ablation study)
    Efficiency: Depthwise separable convolutions reduce params by ~9x vs standard Conv2D

    This model uses MobileNet-inspired depthwise separable convolutions
    for maximum parameter efficiency while maintaining accuracy.
    """
    inputs = layers.Input(shape=input_shape)

    # Initial standard convolution (64×300 → 64×300)
    x = layers.Conv2D(32, (3, 3), padding='same', activation='relu6')(inputs)
    x = layers.Dropout(dropout * 0.5)(x)

    # Block 1: Depthwise separable → 64 filters (64×300 → 32×150)
    x = layers.DepthwiseConv2D((3, 3), padding='same', activation='relu6')(x)
    x = layers.Conv2D(64, (1, 1), padding='same', activation='relu6')(x)
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Dropout(dropout * 0.5)(x)

    # Block 2: Depthwise separable → 128 filters (32×150 → 16×75)
    x = layers.DepthwiseConv2D((3, 3), padding='same', activation='relu6')(x)
    x = layers.Conv2D(128, (1, 1), padding='same', activation='relu6')(x)
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Dropout(dropout * 0.5)(x)

    # Block 3: Depthwise separable → 256 filters (16×75 → 8×37)
    x = layers.DepthwiseConv2D((3, 3), padding='same', activation='relu6')(x)
    x = layers.Conv2D(256, (1, 1), padding='same', activation='relu6')(x)
    x = layers.MaxPooling2D((2, 2))(x)
    ...
    return keras.Model(inputs, outputs, name="Mygardenbird_MobileNetCNN")
...
def main():
    ...
    print(f"  Model: Depthwise Separable CNN (2b)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING MOBILENET-STYLE CNN MODEL (Ablation Study 2b)")
    print(f"{'=' * 70}")
    model = create_mobilenet_cnn(num_classes, config['input_shape'],
                             config['dropout'])
    model.summary()
