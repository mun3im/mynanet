#!/usr/bin/env python3
"""
Model 4b - TCN Shallow: Reduced depth (5 blocks instead of 7)
- Purpose: Test if full receptive field coverage is needed
- Blocks: 5 (dilations: 1,2,4,8,16) vs baseline 7
- Receptive field: ~65 time steps (vs 509 in baseline)
- Hypothesis: Shallow network faster but may lose long-range dependencies
Model 4b - TCN Shallow: Reduced depth (5 blocks instead of 7)
- Purpose: Test if full receptive field coverage is needed
- Blocks: 5 (dilations: 1,2,4,8,16) vs baseline 7
- Receptive field: ~65 time steps (vs 509 in baseline)
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4b - TCN Shallow)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Temporal Convolutional Network (TCN) - SHALLOW VARIANT\n")
            ...
            f.write(f"  Dilation Rates:         [1, 2, 4, 8, 16] per block (SHALLOW: 5 instead of 6)\n")
            f.write(f"  Receptive Field:        ~128 timesteps (vs ~256 in baseline)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  ✓ Shallow TCN (5 dilation blocks instead of 6)\n")
            f.write(f"  - Causal dilated convolutions: [1,2,4,8,16] × 2 blocks\n")
            f.write(f"  - Residual + skip connections\n")
            f.write(f"  - ReLU6 activation (quantization-friendly)\n")
            f.write(f"  - Reduced receptive field: ~128 timesteps\n")
...
# --------------------------------------------------------------
# TCN MODEL - SHALLOW VARIANT (5 blocks instead of 6)
# --------------------------------------------------------------
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    """Create SHALLOW TCN model for mygardenbird classification.

    Modification: Uses dilations [1,2,4,8,16] instead of [1,2,4,8,16,32]
    This reduces the receptive field to ~128 timesteps (vs ~256 in baseline)
    """
    ...
    skips = []
    for _ in range(2):
        # SHALLOW: Only 5 dilation rates instead of 6
        for d in [1, 2, 4, 8, 16]:
            res = x
    ...
    return keras.Model(inputs, outputs, name="Mygardenbird_TCN_Shallow")
...
def main():
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING SHALLOW TCN MODEL (5 blocks instead of 6)")
    print(f"{'=' * 70}")
