#!/usr/bin/env python3
"""
MobileNetV3Small Width-Multiplier Sweep — Series 3
...
Variants: width_mult ∈ {0.5, 0.75, 1.0}

Input pipeline (inside model, as Keras layers):
  (n_mels, time_frames, 1)
  → Lambda: tf.repeat(x, 3, axis=-1)   # replicate to 3-channel for MobileNetV3
  → MobileNetV3Small(include_top=False, weights=None, alpha=width_mult)
...
def get_config():
    ...
    parser.add_argument("--width_mult", type=float, default=0.5,
                        ...
                        help="MobileNetV3Small width multiplier (0.5, 0.75, or 1.0)")
    ...
    # Format width_mult as string for dir name (e.g. 0.5 → "0.5")
    wm_str = str(width_mult)
    ...
    config = {
        ...
        'model_type': f'3_mobilenetv3s_w{wm_str}',
...
class TrainingLogger:
    ...
    def __init__(self, output_dir, width_mult):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write(f"MODEL 3: MobileNetV3Small (width_mult={width_mult}, from scratch)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MobileNetV3Small (Series 3)\n")
            f.write(f"  Width Multiplier:       {config['width_mult']}\n")
            f.write(f"  Pretrained Weights:     None (training from scratch — audio domain)\n")
            f.write(f"  Input Replication:      1-channel → 3-channel (Lambda repeat)\n")
            f.write(f"  Classification Head:    Dense(128,relu) + BN + Dropout → Dense(n_classes,softmax)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        wm_str = str(config['width_mult'])

        with open(self.log_path, 'a') as f:
            f.write("\n" + "=" * 80 + "\n")
            f.write("QUICK REFERENCE (Copy to spreadsheet)\n")
            f.write("=" * 80 + "\n")
            f.write(f"Config: 3_mobilenetv3s_w{wm_str}_drp{int(config['dropout'] * 10)}_"
            ...
            f.write("=" * 80 + "\n")
            f.write("model_type,width_mult,dropout,augmentation,warmup_epochs,finetune_epochs,"
                    "warmup_lr,finetune_lr,lr_schedule,fp32_acc,int8_acc,drop,"
                    "best_val_acc,train_val_gap,train_time_sec,model_size_kb\n")
            f.write(f"3_mobilenetv3s_w{wm_str},{config['width_mult']},{config['dropout']},"
                    f"{config['augmentation_mode']},"
                    f"{config['warmup_epochs']},{config['finetune_epochs']},"
                    f"{config['warmup_lr']},{config['finetune_lr']},"
...
# --------------------------------------------------------------
def create_mobilenetv3s(num_classes, input_shape, width_mult=0.5, dropout=0.05):
    """
    MobileNetV3Small from scratch for mel-spectrogram classification.

    Architecture:
      Input: (n_mels, time_frames, 1)
      Lambda: tf.repeat(x, 3, axis=-1)   # 1-ch → 3-ch replication
      MobileNetV3Small(include_top=False, weights=None, alpha=width_mult)
      GlobalAveragePooling2D
      Dense(128, relu) + BatchNormalization + Dropout
      Dense(n_classes, softmax)

    No pretrained weights — audio domain, training from scratch.
    """
    n_mels, time_frames, _ = input_shape

    # Functional model: Input → Lambda expand → MBV3Small → head
    inputs = layers.Input(shape=input_shape, name='input')

    # Replicate single channel to 3 channels for MobileNetV3 compatibility
    x = layers.Lambda(
        lambda t: tf.repeat(t, 3, axis=-1),
        name='channel_replicate'
    )(inputs)

    # MobileNetV3Small backbone (from scratch, no pretrained weights)
    ...
    return keras.Model(inputs, outputs, name=f"MobileNetV3Small_w{width_mult}")


# We need tf_keras.applications at module level
import tf_keras as tf_keras_apps
...
def main():
    ...
    logger = TrainingLogger(config['output_dir'], width_mult)
    ...
    print(f"  Model: MobileNetV3Small (from scratch, width_mult={width_mult})")
    ...
    print(f"\n{'=' * 70}")
    print(f"CREATING MobileNetV3Small MODEL (Series 3, width_mult={width_mult}) — from scratch")
    print(f"{'=' * 70}")
    model = create_mobilenetv3s(num_classes, config['input_shape'],
                                width_mult=width_mult, dropout=config['dropout'])
    model.summary()
    ...
    print(f"\n{'=' * 70}")
    print(f"FINAL RESULTS — MobileNetV3Small (Series 3, width_mult={width_mult})")
    print(f"{'=' * 70}")
    print(f"Width Multiplier:        {width_mult}")
