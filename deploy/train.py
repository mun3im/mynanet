...
MobileNetV3-Small improvements over 1i (MBV2+SE):
  1. Hard-sigmoid in SE excitation (relu6(x+3)/6 instead of sigmoid)
     - ReLU6-based approximation: better INT8 quantization behaviour
     - All ops basic: Add, ReLU6, Mul — no exponential, fully MCU-safe
  2. 5×5 depthwise convolutions in blocks 3 and 4 (deeper layers only)
     - Wider receptive field for temporal/spectral patterns in mel-spectrograms
     - Marginal parameter increase (depthwise kernels are tiny)
     - Blocks 1–2 keep 3×3 (avoid overfitting on small feature maps)

Model Architecture: MBV2 inverted residual + SE + MBV3 micro-improvements
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 1J: MBV3-SE (5x5 depthwise blocks 3-4, hard-sigmoid SE)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"1j_mbv3_se,{config['dropout']},{config['augmentation_mode']},"
...
# --------------------------------------------------------------
# INVERTED RESIDUAL + SE BLOCKS (Model 1j — MobileNetV3-SE)
# --------------------------------------------------------------
...
def create_mbv3_se(num_classes, input_shape, dropout=0.2):
    """
    MobileNetV3-SE Inverted Residual (Model 1j).

    Architecture:
      Input: (n_mels, 300, 1) mel-spectrogram
      Conv2D(32, 3×3) + BN + ReLU6                          # Stem
      InvRes-SE(t=1, 32→16,  dw=3×3) + MaxPool2D + Dropout # Block 1
      InvRes-SE(t=6, 16→24,  dw=3×3) + MaxPool2D + Dropout # Block 2
      InvRes-SE(t=6, 24→48,  dw=5×5) + MaxPool2D + Dropout # Block 3  ← 5×5
      InvRes-SE(t=6, 48→96,  dw=5×5) + MaxPool2D + Dropout # Block 4  ← 5×5
      Conv2D(320, 1×1) + BN + ReLU6                         # Last-stage expansion
      GlobalAveragePooling2D
      Dense(128, relu6) + Dropout + Dense(n_classes, softmax)

    Changes from 1i (MBV2+SE):
    - SE hard-sigmoid (relu6(x+3)/6) instead of sigmoid — better INT8 quantization
    - 5×5 depthwise in blocks 3-4: wider receptive field for spectral/temporal patterns
    - Blocks 1-2 keep 3×3: small feature maps don't benefit from larger kernels
    - All ops H7-compatible: Conv2D, DepthwiseConv2D, BN, ReLU6, Add, Mul, Lambda(relu6)
    - Hard-sigmoid Lambda decomposes to Add+ReLU6+Mul — no FLEX ops needed

    Estimated ~270 KB INT8 (within H7 512KB flash limit).
    """
    ...
    # Block 4: t=6, 48→96, dw=5×5 (MBV3 wider receptive field)
...
def main():
    ...
    print(f"{'=' * 70}")
    model = create_mbv3_se(num_classes, config['input_shape'],
                           config['dropout'])
    model.summary()
