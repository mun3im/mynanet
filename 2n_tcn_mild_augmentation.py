#!/usr/bin/env python3
"""
Model 4r - TCN with MILD SpecAugment + Mixup (Improved Augmentation)
- Based on 4o SE-TCN (best TCN at 86.22%)
- Architecture: TCN + SE blocks for channel attention
- Kernel: 5 (proven best)
- Channels: 64 (optimal for size)
- IMPROVEMENT: Combined MILD SpecAugment + Mixup
  * SpecAugment was TOO aggressive in 4m (failed at 10% accuracy)
  * This uses 50% milder SpecAugment + proven Mixup combination
- SE blocks: Adaptive channel recalibration
- Goal: 87-88% accuracy (beating 4o's 86.22%)
- Expected size: ~760 KB (same as 4o)
"""
...
# SpecAugment settings - MILD (50% of 4m's aggressive settings)
# 4m failed with freq=8, time=20, masks=2 (too aggressive!)
# These milder settings should work with Mixup combination
SPECAUGMENT_FREQ_MASK = 4   # 6.25% of 64 bins (vs 12.5% in 4m)
SPECAUGMENT_TIME_MASK = 10  # 3.3% of 300 frames (vs 6.7% in 4m)
SPECAUGMENT_NUM_MASKS = 1   # Apply once (vs twice in 4m)
...
def get_config():
    ...
    parser.add_argument("--tcn_channels", type=int, default=64, help="TCN channel width (64=best size/acc tradeoff, 96, 128)")
    ...
    parser.add_argument("--combined", action='store_true',
                        help="Enable COMBINED augmentation (mild SpecAugment + Mixup) - RECOMMENDED!")
    ...
    mixup_alpha = None  # Initialize here

    if args.combined:
        augmentation_mode = "combined"
        aug_suffix = "mild_specaug_mixup0.2"
        # Set mixup alpha for combined mode
        mixup_alpha = 0.2
    elif args.mixup is not None:
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4r - TCN with MILD SpecAugment + Mixup)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Temporal Convolutional Network (TCN) - SE VARIANT\n")
            ...
            elif config['augmentation_mode'] == 'combined':
                f.write(f"  Type:                   COMBINED (Mild SpecAugment + Mixup)\n")
                f.write(f"  SpecAugment Settings:\n")
                f.write(f"    - Frequency Mask:     {SPECAUGMENT_FREQ_MASK} bins (50% milder than 4m)\n")
                f.write(f"    - Time Mask:          {SPECAUGMENT_TIME_MASK} frames (50% milder than 4m)\n")
                f.write(f"    - Number of Masks:    {SPECAUGMENT_NUM_MASKS} (vs 2 in 4m)\n")
                f.write(f"  Mixup Settings:\n")
                f.write(f"    - Alpha:              {config['mixup_alpha']}\n")
                f.write(f"  Data Multiplier:        2x (original + SpecAug'd) + Mixup\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            elif config['augmentation_mode'] in ['mixup', 'combined']:
                f.write(f"  Alpha: {config['mixup_alpha']}\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    ...
    return keras.Model(inputs, outputs, name="Mygardenbird_SE_TCN")
...
def main():
    ...
    if config['augmentation_mode'] in ['mixup', 'combined']:
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING SE-TCN MODEL WITH ATTENTION (kernel_size=5, channels=64)")
    print(f"{'=' * 70}")
