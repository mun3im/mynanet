...
def create_mobilenetv1_narrow_64x300(num_classes, input_shape, width_mult=0.35, dropout=0.5, l2_reg=1e-4):
    """
    Create Regularized MobileNetV1 adapted for 64x300 spectrogram input.

    Model 7b: V1.5 upgrade with strong regularization to combat overfitting.

    Architecture based on MobileNetV1 (Howard et al., 2017):
    ...
    Args:
        ...
        dropout: Dropout rate (default 0.5 for 7b)
        l2_reg: L2 regularization strength (default 1e-4)
    ...
    # Initial conv - stride=1 to preserve 64x300 resolution (with L2 regularization)
    x = layers.Conv2D(
    ...
    # Adapted for 64x300: fewer aggressive downsampling
    block_configs = [
        # filters, stride - produces spatial dimension
        (64,  1),   # 64x300
        (128, 2),   # 32x150
        (128, 1),   # 32x150
        (256, 2),   # 16x75
    ...
    for i, (filters, stride) in enumerate(block_configs):
        x = depthwise_separable_block(x, ch(filters), stride, i)
    ...
    return keras.Model(inputs, outputs, name="MobileNetV1_Regularized_64x300")
...
def main():
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING REGULARIZED MOBILENETV1 MODEL (64x300 NATIVE)")
    print(f"{'=' * 70}")
    model = create_mobilenetv1_narrow_64x300(
