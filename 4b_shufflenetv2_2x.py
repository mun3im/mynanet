#!/usr/bin/env python3
"""
Ablation Study 2g: ShuffleNetV2 with Width Multiplier 2.0 for Mygardenbird Classification
...
Hypothesis: Test if ShuffleNetV2 with wider channels (width_multiplier=2.0) can achieve
higher accuracy while maintaining efficient channel shuffle operations. ShuffleNetV2
offers excellent speed-accuracy tradeoff through channel split and shuffle operations.

Architecture: ShuffleNetV2 with width=2.0 adapted for 64×300
- Width multiplier 2.0 doubles all channel counts for higher capacity
- Channel shuffle for efficient cross-group information flow
- Depthwise separable convolutions for efficiency
- First conv stride=1 (preserve 64×300 resolution)
- Target: ~2-3M parameters (higher capacity than MobileNetV3)
- Expected size: ~600-800 KB INT8

Expected Performance:
- Target accuracy: 90-95% (highest among efficient architectures)
- Model size: ~600-800 KB INT8
- Best speed-accuracy tradeoff with channel shuffle
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 2G: SHUFFLENETV2 WIDTH=2.0 @ 64×300 NATIVE")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             ShuffleNetV2 Width=2.0 (Ablation 2g)")
            f.write(f"  Width Multiplier:       2.0 (double channel capacity)")
            f.write(f"  Input Shape:            {config['input_shape']} (NATIVE, NO RESIZE)\n")
            f.write(f"  Hypothesis:             ShuffleNetV2 with wider channels for higher accuracy")
            ...
            f.write(f"  Target Size:            ~600-800 KB INT8")
            ...
            f.write(f"  Memory Target:          ~600-800 KB (higher capacity)")
    ...
    def log_model_info(self, model):
        ...
        with open(self.log_path, 'a') as f:
            ...
            if int8_size_kb > 800:
                f.write(f"  ⚠ WARNING: Model may exceed 800 KB target")
            else:
                f.write(f"  ✓ Model size within 800 KB target")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 2b_mobilenetv3_64x300 | Dropout: {config['dropout']} | "
            ...
            f.write(f"2g_shufflenetv2_2x,{config['dropout']},{config['augmentation_mode']},"
...
def channel_shuffle(x, groups):
    ...
    x = layers.Permute((1, 2, 4, 3))(x)  # Swap groups and channels_per_group
    x = layers.Reshape((height, width, channels))(x)
    return x


def shufflenet_unit(x, out_channels, stride, block_id):
    ...
    if stride == 1:
        ...
        shortcut = x[:, :, :, :split_channels]
        x = x[:, :, :, split_channels:]

        # Branch
        x = layers.Conv2D(split_channels, 1, use_bias=False, name=prefix + 'conv1')(x)
        x = layers.BatchNormalization(name=prefix + 'bn1')(x)
        x = layers.ReLU(name=prefix + 'relu1')(x)

        x = layers.DepthwiseConv2D(3, padding='same', use_bias=False, name=prefix + 'dwconv')(x)
        x = layers.BatchNormalization(name=prefix + 'bn2')(x)

        x = layers.Conv2D(split_channels, 1, use_bias=False, name=prefix + 'conv2')(x)
        x = layers.BatchNormalization(name=prefix + 'bn3')(x)
        x = layers.ReLU(name=prefix + 'relu2')(x)

        # Concatenate
        x = layers.Concatenate(name=prefix + 'concat')([shortcut, x])
    else:
        # Stride 2: process both branches
        # Branch 1
        shortcut = layers.DepthwiseConv2D(3, strides=stride, padding='same', use_bias=False,
                                         name=prefix + 'shortcut_dwconv')(x)
        shortcut = layers.BatchNormalization(name=prefix + 'shortcut_bn1')(shortcut)
        shortcut = layers.Conv2D(out_channels // 2, 1, use_bias=False, name=prefix + 'shortcut_conv')(shortcut)
        shortcut = layers.BatchNormalization(name=prefix + 'shortcut_bn2')(shortcut)
        shortcut = layers.ReLU(name=prefix + 'shortcut_relu')(shortcut)

        # Branch 2
        x = layers.Conv2D(out_channels // 2, 1, use_bias=False, name=prefix + 'conv1')(x)
        x = layers.BatchNormalization(name=prefix + 'bn1')(x)
        x = layers.ReLU(name=prefix + 'relu1')(x)

        x = layers.DepthwiseConv2D(3, strides=stride, padding='same', use_bias=False, name=prefix + 'dwconv')(x)
        x = layers.BatchNormalization(name=prefix + 'bn2')(x)

        x = layers.Conv2D(out_channels // 2, 1, use_bias=False, name=prefix + 'conv2')(x)
        x = layers.BatchNormalization(name=prefix + 'bn3')(x)
        x = layers.ReLU(name=prefix + 'relu2')(x)
...
def create_shufflenetv2_64x300(num_classes, input_shape, dropout=0.2, width_mult=2.0):
    """
    ShuffleNetV2 with width multiplier adapted for 64×300 spectrogram input.

    Key adaptations:
    1. Width multiplier 2.0 doubles all channel counts
    2. First conv stride=1 instead of 2 (preserve 64×300 resolution)
    3. Channel shuffle for efficient information flow
    4. Depthwise separable convolutions

    Architecture based on ShuffleNetV2 from:
    "ShuffleNet V2: Practical Guidelines for Efficient CNN Architecture Design" (Ma et al., 2018)
    """
    ...
    # Initial conv - stride=1 to preserve resolution
    init_filters = _make_divisible(24 * width_mult)
    x = layers.Conv2D(init_filters, 3, strides=1, padding='same', use_bias=False, name='conv1')(inputs)
    x = layers.BatchNormalization(name='bn1')(x)
    x = layers.ReLU(name='relu1')(x)

    # Stage configurations: (num_blocks, out_channels, stride_first_block)
    stage_configs = [
        (4,  _make_divisible(116 * width_mult), 2),  # Stage 2: 32×150
        (8,  _make_divisible(232 * width_mult), 2),  # Stage 3: 16×75
        (4,  _make_divisible(464 * width_mult), 2),  # Stage 4: 8×37
    ]

    block_id = 0
    for num_blocks, out_channels, first_stride in stage_configs:
        for i in range(num_blocks):
            stride = first_stride if i == 0 else 1
            x = shufflenet_unit(x, out_channels, stride, block_id)
            block_id += 1

    # Final conv
    final_filters = _make_divisible(1024 * width_mult)
    x = layers.Conv2D(final_filters, 1, use_bias=False, name='conv5')(x)
    ...
    return keras.Model(inputs, outputs, name="ShuffleNetV2_Width2x_64x300")
...
def main():
    ...
    print(f"Ablation 2g: ShuffleNetV2 Width=2.0 @ 64×300 (NATIVE)")
    print(f"  Hypothesis: ShuffleNetV2 with wider channels for higher accuracy")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING SHUFFLENETV2 WIDTH=2.0 MODEL (64×300 NATIVE)")
    print(f"{'=' * 70}")
    model = create_shufflenetv2_64x300(num_classes, config['input_shape'], config['dropout'], width_mult=2.0)
    model.summary()
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   2g_shufflenetv2_2x (64×300 NATIVE)")
    print(f"Width Multiplier:        2.0")
    print(f"Augmentation:            {config['augmentation_mode']}")
    ...
    print(f"Speed-Accuracy Tradeoff: ShuffleNetV2 with channel shuffle for efficiency")
    print(f"{'=' * 70}")
