#!/usr/bin/env python3
"""
DS-CNN + SE + Residual + Wide, no Attention -> Model 1f (MCU Compatible)
...
Model Architecture: DS-CNN with SE and Residual in ALL blocks, wide channels, no attention (~MCU Compatible)
  ...
  GlobalAveragePooling2D (replaces MHSA — H7 compatible)
...
Key Improvements over Model 1d:
  ...
  3. No attention (MHSA replaced with GAP for H7 compatibility)
     - Only TFLite Micro supported ops
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             DS-CNN + SE + Residual (All) + Wide, no Attention (Model 1f)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"1f_dscnn_se_res_wide,{config['dropout']},{config['augmentation_mode']},"
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  DS-CNN + SE + Residual (All) + Wide, no Attention (Model 1f)\n")
            ...
            f.write(f"  - No MHSA: replaced with GlobalAveragePooling2D for H7 compatibility\n")
            f.write(f"  - GlobalAveragePooling2D + Dense(192) classifier\n")
...
def create_se_res_att(num_classes, input_shape, dropout=0.2, channels=None):
    """
    DS-CNN + SE + Residual (ALL blocks) + Wide, no Attention (Model 1f)
    ...
    # GAP — replaces MHSA; uses only ops supported by TFLite Micro on H7
    ...
    return keras.Model(inputs, outputs, name="MynaNet_DSCNN_SE_Res_Wide")
...
def main():
    ...
    print(f"  Model: DS-CNN + SE + Residual (All) + Wide, no Attention (1h)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING DS-CNN + SE + RESIDUAL (ALL) + ATTENTION + WIDER (Model 1f)")
    print(f"{'=' * 70}")
    print(f"  Channels: {config['channels']}")
