#!/usr/bin/env python3
"""
Ablation Study 2f: Optimized MobileNetV3-Small for Mygardenbird Classification
Fixed spectrogram shape: 80x300 (10ms per frame) - NATIVE INPUT, NO RESIZE
Data Split: CSV-based splits (75:10:15, 80:10:10, 70:15:15)

Optimizations vs 2e:
1. Width multiplier 0.75 (reduce params from 1.7M → ~800K)
2. Stronger regularization (dropout 0.3, spatial dropout 0.1, mixup)
3. Reduced classification head (1024 → 512)
4. Better training schedule (longer warmup, lower LR)
5. CSV-based data loading for reproducible splits

Architecture: MobileNetV3-Small-0.75 adapted for 80×300
- Keep: SE blocks, hard-swish, inverted residuals
- Optimize: Channel counts, dropout, head size
- Target: <1MB INT8, >85% accuracy, stable baseline
"""
...
if platform.system() == "Darwin":
    DEFAULT_CSV_DIR = "/Users/mun3im/Dropbox/Conda/SEAbird/mygardenbird_validation/splits_csv"
else:
    DEFAULT_CSV_DIR = "/home/muneim/Dropbox/Conda/SEAbird/mygardenbird_validation/splits_csv"
...
def get_config():
    ...
    parser.add_argument("--width_multiplier", type=float, default=0.75)
    parser.add_argument("--calib_samples", type=int, default=200)

    # CSV split selection
    parser.add_argument("--split_csv", type=str, required=True,
                        choices=['75_10_15', '80_10_10', '70_15_15'],
                        help="CSV split ratio to use")

    # Augmentation flags
    parser.add_argument("--augment", action='store_true',
                        help="Enable baseline augmentation (time/pitch shift)")
    parser.add_argument("--mixup", type=float, default=0.2,
                        help="Enable mixup augmentation with alpha value (default: 0.2)")
    ...
    parser.add_argument("--csv_dir", type=str, default=DEFAULT_CSV_DIR,
                        help="Path to CSV splits directory")

    # LR schedule
    parser.add_argument("--lr_schedule", type=str, default="both",
    ...
    output_dir_name = (
        f"results/2f_mobilenetv3_opt_w{int(args.width_multiplier*100)}_"
        f"split{args.split_csv}_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 2F: OPTIMIZED MOBILENETV3SMALL @ 80×300 (CSV SPLITS)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"  FFT Window:             Hann (YAMNet standard)\n")
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MobileNetV3-Small-{config['width_multiplier']}\n")
            f.write(f"  Input Shape:            {config['input_shape']} (NATIVE, NO RESIZE)\n")
            f.write(f"  Width Multiplier:       {config['width_multiplier']}\n")
            f.write(f"  Dropout Rate:           {config['dropout']}\n")
            f.write(f"  Spatial Dropout:        {config['spatial_dropout']}\n")
            ...
            f.write(f"  Classification Head:    512 (reduced from 1024)\n")
            f.write(f"  Quantization Target:    INT8 optimized architecture\n")

            f.write("\nTraining Configuration:\n")
            f.write(f"  Random Seed:            {config['random_seed']}\n")
            f.write(f"  CSV Split:              {config['split_csv']}\n")
            ...
            f.write(f"  Loss Function:          Categorical Crossentropy (Mixup)\n")
            ...
            if config['augmentation_mode'] == 'mixup':
                ...
                f.write(f"  Data Multiplier:        On-the-fly (no disk space)\n")
            else:
            ...
            f.write(f"  CSV Directory:          {config['csv_dir']}\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 2f_mobilenetv3_opt_w{int(config['width_multiplier']*100)} | "
                    f"Split: {config['split_csv']} | Dropout: {config['dropout']} | "
                    f"Aug: {config['augmentation_mode']}\n")
            ...
            f.write("=" * 80 + "\n")
            f.write("model,width_mult,split,dropout,augmentation,warmup_epochs,finetune_epochs,warmup_lr,finetune_lr,"
                    "lr_schedule,fp32_acc,int8_acc,drop,best_val_acc,train_val_gap,train_time_sec,model_size_kb\n")
            f.write(f"2f_mobilenetv3_opt,{config['width_multiplier']},{config['split_csv']},{config['dropout']},{config['augmentation_mode']},"
...
def load_csv_split(csv_path):
    ...
    print(f"✓ Loaded CSV split: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}")
    return splits


def load_data_from_csv(data_dir, csv_path, gmin, gmax, augmentation_mode='mixup', mixup_alpha=0.2):
    """Load data using CSV split file."""
    print(f"\nLoading dataset from CSV: {csv_path}")

    splits = load_csv_split(csv_path)
    ...
    # Build class labels from directory structure
    for class_name in sorted(os.listdir(data_dir)):
        class_dir = os.path.join(data_dir, class_name)
    ...
    with tqdm(total=total_files, desc="Loading audio files", unit="file") as pbar:
        # Process all splits
        for split_name, file_list in [('test', splits['test']), ('val', splits['val']), ('train', splits['train'])]:
            for filename in file_list:
                # Extract class from filename (format: xc123456_789.wav -> class from directory)
                found = False
                for class_name in labels.keys():
                    audio_path = os.path.join(data_dir, class_name, filename)
                    if os.path.exists(audio_path):
                        try:
                            ...
                            failed_files.append(f"{audio_path}: {str(e)}")
                            break

                if not found:
                    failed_files.append(f"{filename}: not found in any class directory")
    ...
    if len(failed_files) > 0:
        ...
        with open('data_loading_errors_csv.txt', 'w') as f:
            for error in failed_files:
                f.write(error + '\n')

    print(f"\n✓ CSV-based Split Complete:")
    ...
    print(f"\n  ✓ CSV-based splits ensure reproducibility across experiments")
...
# --------------------------------------------------------------
# MOBILENETV3SMALL OPTIMIZED FOR 80×300 INPUT
# --------------------------------------------------------------
def hard_swish(x):
...
def inverted_residual_block(x, expansion, filters, kernel_size, stride, se_ratio, activation,
                           block_id, spatial_dropout=0.0):
    """Inverted residual block with optional spatial dropout"""
    shortcut = x
    ...
    # Spatial dropout before SE block
    if spatial_dropout > 0:
        x = layers.SpatialDropout2D(spatial_dropout, name=prefix + 'spatial_dropout')(x)
...
def create_mobilenetv3_small_optimized(num_classes, input_shape, dropout=0.3,
                                      spatial_dropout=0.1, width_multiplier=0.75):
    """
    Optimized MobileNetV3-Small for 80×300 spectrogram input.

    Optimizations vs 2e:
    1. Width multiplier to reduce params (0.75 default)
    2. Spatial dropout for regularization (0.1)
    3. Reduced classification head (512 instead of 1024)
    4. Keep all SE blocks and architectural innovations

    Target: ~800K params, <1MB INT8, >85% accuracy
    """
    ...
    def _ch(channels):
        return _make_divisible(channels * width_multiplier)

    # Initial conv - stride=1 to preserve 80×300 resolution
    x = layers.Conv2D(_ch(16), 3, strides=1, padding='same', use_bias=False, name='conv_stem')(inputs)
    x = layers.BatchNormalization(name='conv_stem_bn')(x)
    x = layers.Activation(hard_swish, name='conv_stem_act')(x)

    # MobileNetV3Small blocks with width multiplier
    # Format: expansion, filters, kernel, stride, se_ratio, activation
    block_configs = [
        # exp, filters, kernel, stride, se,    act
        (1,   16,      3,      2,      4,     'relu'),        # 40×150
        (4.5, 24,      3,      2,      None,  'relu'),        # 20×75
        (3.67,24,      3,      1,      None,  'relu'),        # 20×75
        (4,   40,      5,      2,      4,     hard_swish),    # 10×38
        (6,   40,      5,      1,      4,     hard_swish),    # 10×38
        (6,   40,      5,      1,      4,     hard_swish),    # 10×38
        (3,   48,      5,      1,      4,     hard_swish),    # 10×38
        (3,   48,      5,      1,      4,     hard_swish),    # 10×38
        (6,   96,      5,      2,      4,     hard_swish),    # 5×19
        (6,   96,      5,      1,      4,     hard_swish),    # 5×19
        (6,   96,      5,      1,      4,     hard_swish),    # 5×19
    ]

    for i, (exp, filters, kernel, stride, se, act) in enumerate(block_configs):
        x = inverted_residual_block(x, exp, _ch(filters), kernel, stride, se, act, i,
                                   spatial_dropout=spatial_dropout)

    # Final conv
    x = layers.Conv2D(_ch(576), 1, padding='same', use_bias=False, name='conv_head')(x)
    ...
    x = layers.Reshape((1, 1, _ch(576)))(x)

    # OPTIMIZED: Reduced classification head (512 instead of 1024)
    x = layers.Conv2D(_ch(512), 1, use_bias=True, name='conv_final_1')(x)
    ...
    return keras.Model(inputs, outputs, name=f"MobileNetV3Small_w{int(width_multiplier*100)}_80x300")
...
def main():
    ...
    print(f"\nAblation 2f: Optimized MobileNetV3-Small @ 80×300")
    print(f"  Width multiplier: {config['width_multiplier']}")
    print(f"  CSV split: {config['split_csv']}")
    print(f"  Random seed: {config['random_seed']}")
    print(f"  Warmup epochs: {config['warmup_epochs']}")
    print(f"  Dropout: {config['dropout']}")
    print(f"  Spatial dropout: {config['spatial_dropout']}")
    ...
    # Load data from CSV
    csv_filename = f"mygardenbird_splits_{config['split_csv']}_seed{config['random_seed']}.csv"
    csv_path = os.path.join(config['csv_dir'], csv_filename)

    print(f"\nLoading dataset from CSV split: {config['split_csv']}")
    X_train, X_val, X_test, y_train, y_val, y_test, class_labels, failed_count = load_data_from_csv(
        config['dataset_dir'], csv_path, global_min, global_max,
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print(f"CREATING OPTIMIZED MOBILENETV3SMALL MODEL (w={config['width_multiplier']})")
    print(f"{'=' * 70}")
    model = create_mobilenetv3_small_optimized(
        num_classes, config['input_shape'],
        config['dropout'], config['spatial_dropout'],
    ...
    # Prepare training data (always use mixup for this optimized version)
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   2f_mobilenetv3_opt (w={config['width_multiplier']})")
    print(f"CSV Split:               {config['split_csv']}")
    print(f"Augmentation:            {config['augmentation_mode']}")
