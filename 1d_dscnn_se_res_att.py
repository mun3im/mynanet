...
def get_config():
    ...
    output_dir_name = (
        ...
        f"1d_dscnn_se_res_att{wide_suffix}_"
        f"mels{n_mels}_"
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             DS-CNN + SE + Residual + Attention{wide_str} (Model 1d)\n")
...
def main():
    ...
    print(f"\n{'=' * 70}")
    print(f"CREATING DS-CNN + SE + RESIDUAL MODEL + ATTENTION{wide_str} (Model 1d)")
    print(f"{'=' * 70}")
    print(f"  Channels: {config['channels']}")
