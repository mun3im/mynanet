#!/usr/bin/env python3
"""
Model 4n - TCN Combined: TCN with SpecAugment + Mixup
- Based on 4k optimized variant
- Kernel: 5 (proven best from ablation studies)
- Channels: 96 (optimized capacity)
- Augmentation: SpecAugment + Mixup (combined for maximum regularization)
- Goal: Achieve highest accuracy through combined augmentation
- Expected: 88-92% accuracy (vs 4f's 81.56%)
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4n - TCN Combined Augmentation)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Temporal Convolutional Network (TCN) - COMBINED AUGMENTATION\n")
...
def main():
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING TCN MODEL WITH COMBINED AUGMENTATION (kernel_size=5, channels=96)")
    print(f"{'=' * 70}")
