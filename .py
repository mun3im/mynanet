#!/usr/bin/env python3
"""
Post-Training Quantization (PTQ) for Seabird TCN → Cortex-M7 deployment
Fixed spectrogram shape: 64x300 (10ms per frame)
Data Split: 90 test / 60 val / 450 train per class (600 total)

Model 4a - TCN Baseline: Best of 7c + 4d
- All 7c features: SpecAugment, Mixup, fixed per-class split, WIN_LENGTH, enhanced logging
- All 4d features: smart cache, configurable LR schedule, configurable seed/paths
- Early validation: Check Keras/GPU before dataset preparation
- Single format: .keras only (no .h5)
"""
...
def get_config():
    ...
    output_dir_name = (
        f"results/4a_tcn_64x300_ptq_"
        f"chan{args.tcn_channels}_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("SEABIRD TCN TRAINING REPORT (Model 4a - TCN Baseline)\n")
            f.write("=" * 80 + "\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    """Create TCN model for seabird classification."""
    ...
    return keras.Model(inputs, outputs, name="Seabird_TCN")
...
def load_data(data_dir, gmin, gmax, augmentation_mode='none',
    ...
    # Add augmented samples to total if enabled (not for mixup - done during training)
    train_count = total_files - ((TEST_SIZE_PER_CLASS + VAL_SIZE_PER_CLASS) * 10)
    if augmentation_mode in ['baseline', 'specaugment']:
        total_files += train_count

    print(f"\nDataset Split Strategy (600 samples/class):")
    print(f"  Test set: {TEST_SIZE_PER_CLASS} samples/class (held-out, never augmented)")
    print(f"  Val set: {VAL_SIZE_PER_CLASS} samples/class (held-out, never augmented)")

    if augmentation_mode == 'baseline':
        print(f"  Train set: 450 samples/class + baseline augmentation (time/pitch shift)")
    elif augmentation_mode == 'mixup':
        print(f"  Train set: 450 samples/class (mixup applied during training, alpha={mixup_alpha})")
    elif augmentation_mode == 'specaugment':
        print(f"  Train set: 450 samples/class + SpecAugment (freq/time masking)")
    else:
        print(f"  Train set: 450 samples/class (no augmentation)")

    print(f"  Total samples to process: {total_files}")

    with tqdm(total=total_files, desc="Loading with fixed split", unit="file") as pbar:
        for class_name in sorted(os.listdir(data_dir)):
            class_dir = os.path.join(data_dir, class_name)
            if not os.path.isdir(class_dir) or class_name.startswith('.'):
                continue
            ...
            # Reproducible split: set seed based on class name
            class_seed = hash(class_name) % (2 ** 32)
            np.random.seed(class_seed)
            np.random.shuffle(files)

            # Split: first 90 for test, next 60 for val, rest for train
            test_files = files[:TEST_SIZE_PER_CLASS]
            val_files = files[TEST_SIZE_PER_CLASS:TEST_SIZE_PER_CLASS + VAL_SIZE_PER_CLASS]
            train_files = files[TEST_SIZE_PER_CLASS + VAL_SIZE_PER_CLASS:]
            ...
            # Load train set (with optional augmentation)
