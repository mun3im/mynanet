...
def get_config():
    ...
    import re as _re
    _m = _re.search(r'(\d+)_(\d+)_(\d+)$', Path(args.splits_csv).stem)
    split_label = f"{_m.group(1)}:{_m.group(2)}:{_m.group(3)}" if _m else "custom"
    out_dir = (
        f"results_mygardenbird_1_{plat}/"
        f"1r_efficientnetb0_mels{args.n_mels}_rand{args.random_seed}"
        f"_{aug_mode}_split{split_label}_{plat}"
    )
    return {
