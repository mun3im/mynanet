#!/usr/bin/env python3
"""
Model 7a - Narrow MobileNetV1 for Mygardenbird Classification
...
Architecture: MobileNetV1 with narrow width multiplier
...
- No inverted residual structure (simpler)
- No squeeze-excitation blocks
- No skip connections in standard form
- Direct depthwise → pointwise convolution pattern
...
def get_config():
    ...
    output_dir_name = (
        f"results/7c_mobilenetv1_narrow_regularized_v2_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 7A: NARROW MOBILENETV1 @ 64x300 NATIVE\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MobileNetV1 Narrow (Model 7a)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 7a_mobilenetv1 | Width: {config['width_mult']} | Dropout: {config['dropout']} | "
            ...
            f.write(f"7a_mobilenetv1,{config['width_mult']},{config['dropout']},{config['augmentation_mode']},"
...
# --------------------------------------------------------------
def depthwise_separable_block(x, filters, stride, block_id):
    """
    MobileNetV1 depthwise separable convolution block.

    ENHANCED: Added L2 regularization + stronger dropout-like effect via gamma regularization
    """
    ...
    x = layers.BatchNormalization(
        gamma_regularizer=keras.regularizers.l2(1e-4),  # L2 on BN gamma
        beta_regularizer=keras.regularizers.l2(1e-4),  # L2 on BN beta
...
def create_mobilenetv1_narrow_64x300(num_classes, input_shape, width_mult=0.35, dropout=0.3):
    """
    Create Narrow MobileNetV1 adapted for 64x300 spectrogram input.
    ...
    return keras.Model(inputs, outputs, name="MobileNetV1_Narrow_64x300")
...
def main():
    ...
    print(f"\nModel 7b: Narrow MobileNetV1 @ 64x300 (NATIVE)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING NARROW MOBILENETV1 MODEL (64x300 NATIVE)")
    print(f"{'=' * 70}")
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   7a_mobilenetv1 (64x300 NATIVE)")
    print(f"Width Multiplier:        {config['width_mult']}")
    print(f"Augmentation:            {config['augmentation_mode']}")
