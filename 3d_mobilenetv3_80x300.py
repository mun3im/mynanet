#!/usr/bin/env python3
"""
Ablation Study 2e: MobileNetV3Small for Mygardenbird Classification
...
Hypothesis: Testing with MORE frequency resolution (80 mel bins vs 64)
to determine if additional frequency information improves accuracy.

Architecture: MobileNetV3Small adapted for 80×300
- Remove first stride-2 conv (preserve resolution)
- Adjust spatial reduction to account for 80×300 shape
- Keep depthwise separable blocks (efficient)
- Keep SE (Squeeze-Excitation) blocks (channel attention)
- Target: ~1-2M parameters (comparable to TCN-96)

Version 2e: Based on 7d pipeline with MobileNetV3Small
- Same preprocessing: 7d's mel-spectrogram pipeline
- Same data split: Per-class 90/60/450 (no leakage)
...
DEFAULT_SPECTROGRAM_DIR = "/Volumes/Evo/precompute/mygardenbird_spectrograms_80x300"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 2D: MOBILENETV3SMALL @ 80×300 NATIVE (Version 2d)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 2e_mobilenetv3_80x300 | Dropout: {config['dropout']} | "
            ...
            f.write(f"2d_mobilenetv3,{config['dropout']},{config['augmentation_mode']},"
...
# --------------------------------------------------------------
# MOBILENETV3SMALL ADAPTED FOR 80×300 INPUT
# --------------------------------------------------------------
def hard_swish(x):
...
def create_mobilenetv3_small_80x300(num_classes, input_shape, dropout=0.2):
    """
    MobileNetV3Small adapted for 80×300 spectrogram input (NATIVE, NO RESIZE).

    Key adaptations:
    1. First conv stride=1 instead of 2 (preserve 80×300 resolution)
    ...
    x = layers.Conv2D(16, 3, strides=1, padding='same', use_bias=False, name='conv_stem')(inputs)
    ...
    block_configs = [
        ...
        (1,   16,      3,      2,      4,     'relu'),        # 24×150
        (4.5, 24,      3,      2,      None,  'relu'),        # 12×75
        (3.67,24,      3,      1,      None,  'relu'),        # 12×75
        (4,   40,      5,      2,      4,     hard_swish),    # 6×37
        (6,   40,      5,      1,      4,     hard_swish),    # 6×37
        (6,   40,      5,      1,      4,     hard_swish),    # 6×37
        (3,   48,      5,      1,      4,     hard_swish),    # 6×37
        (3,   48,      5,      1,      4,     hard_swish),    # 6×37
        (6,   96,      5,      2,      4,     hard_swish),    # 3×18
        (6,   96,      5,      1,      4,     hard_swish),    # 3×18
        (6,   96,      5,      1,      4,     hard_swish),    # 3×18
    ]

    for i, (exp, filters, kernel, stride, se, act) in enumerate(block_configs):
        x = inverted_residual_block(x, exp, filters, kernel, stride, se, act, i)

    # Final conv
    x = layers.Conv2D(576, 1, padding='same', use_bias=False, name='conv_head')(x)
    ...
    # Final dense layers
    x = layers.Conv2D(1024, 1, use_bias=True, name='conv_final_1')(x)
    ...
    return keras.Model(inputs, outputs, name="MobileNetV3Small_80x300")
...
def main():
    ...
    print(f"\nAblation 2d: MobileNetV3Small @ 80×300 (NATIVE)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING MOBILENETV3SMALL MODEL (80×300 NATIVE)")
    print(f"{'=' * 70}")
    model = create_mobilenetv3_small_80x300(num_classes, config['input_shape'], config['dropout'])
    model.summary()
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   2d_mobilenetv3 (80×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
    ...
    print(f"Expected: Native 80×300 should preserve resolution better")
    print(f"{'=' * 70}")
