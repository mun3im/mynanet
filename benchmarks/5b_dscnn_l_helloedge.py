#!/usr/bin/env python3
"""
Peer benchmark: DS-CNN-L (Hello Edge, Zhang 2017)
Trained on 12-class MyGardenBird under the same protocol as 1j.
...
# Architecture builder lives in peer_architectures.py (shared across 5a-5d).
from peer_architectures import build_dscnn_l_helloedge
...
def get_config():
    ...
    # Driver-aligned naming: matches PREFIX_OF[5x] in run_peer_benchmarks.sh
    # Expected pattern: 5x_<peer>_drop05_rand<seed>_warm70_mixup0.2_split80:10:10
    output_dir_name = (
        f"results_mygardenbird_5_{platform.system().lower()}/"
        f"5b_dscnn_l_helloedge_"
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             DS-CNN-L (Hello Edge)\n")
            ...
            f.write(f"  Activation:             (see build_dscnn_l_helloedge definition)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 5b_dscnn_l_helloedge | Dropout: {config['dropout']} | "
            ...
            f.write(f"5b_dscnn_l_helloedge,{config['dropout']},{config['augmentation_mode']},"
                    ...
                    f"{os.path.getsize(os.path.join(config['output_dir'], 'dscnn_l_int8.tflite')) / 1024:.1f}\n")
...
def main():
    ...
    print(f"\nPeer 5b_dscnn_l_helloedge: DS-CNN-L (Hello Edge, Zhang 2017)")
    ...
    print(f"{'=' * 70}")
    model = build_dscnn_l_helloedge(num_classes=num_classes, input_shape=config['input_shape'], dropout=config['dropout'])
    model.summary()
    ...
    fp32_path = os.path.join(config['output_dir'], 'dscnn_l_fp32.keras')
    ...
    print(f"{'=' * 70}")
    int8_path = os.path.join(config['output_dir'], 'dscnn_l_int8.tflite')
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   5b_dscnn_l_helloedge (64×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
