#!/usr/bin/env python3
"""
Peer benchmark: TC-ResNet-14-1.5 (Choi 2019)
...
from peer_architectures import build_tcresnet14_15
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             TC-ResNet-14-1.5\n")
            ...
            f.write(f"  Activation:             (see build_tcresnet14_15 definition)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 5d_tcresnet14 | Dropout: {config['dropout']} | "
            ...
            f.write(f"5d_tcresnet14,{config['dropout']},{config['augmentation_mode']},"
                    ...
                    f"{os.path.getsize(os.path.join(config['output_dir'], 'tcresnet14_int8.tflite')) / 1024:.1f}\n")
...
def main():
    ...
    print(f"\nPeer 5d_tcresnet14: TC-ResNet-14-1.5 (Choi 2019)")
    ...
    print(f"{'=' * 70}")
    model = build_tcresnet14_15(num_classes=num_classes, input_shape=config['input_shape'], dropout=config['dropout'])
    model.summary()
    ...
    fp32_path = os.path.join(config['output_dir'], 'tcresnet14_fp32.keras')
    ...
    print(f"{'=' * 70}")
    int8_path = os.path.join(config['output_dir'], 'tcresnet14_int8.tflite')
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   5d_tcresnet14 (64×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
