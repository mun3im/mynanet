#!/usr/bin/env python3
"""
Model 9c - WIDER MatchboxNet for Mygardenbird Classification
...
Architecture: MatchboxNet-3x2x128 WIDER (NVIDIA, 2020)
...
- WIDER version: 128 channels per sub-block (vs 64 in 9a)
- More parameters for increased representation capacity

9c Enhancements (vs 9a):
- Doubled channel width: C=128 (vs C=64)
- Increased capacity for more complex patterns
- Goal: Explore accuracy vs model size tradeoff
...
Target: ARM Cortex-M7, <512KB INT8 model size (wider model may exceed target)
"""
...
# MatchboxNet default configuration - WIDER VERSION
MATCHBOXNET_CONFIG = {
    'B': 3,              # Number of residual blocks
    'R': 2,              # Sub-blocks per residual block
    'C': 128,            # WIDER: 128 channels (vs 64 in 9a)
...
def get_config():
    ...
    parser.add_argument("--matchbox_C", type=int, default=128,
                        help="Channels per sub-block (C in BxRxC) - WIDER: 128 default")
    ...
    # MatchboxNet naming: 9c_matchboxnet_wider_BxRxC
    matchbox_name = f"{args.matchbox_B}x{args.matchbox_R}x{args.matchbox_C}"

    output_dir_name = (
        f"results/9c_matchboxnet_wider_{matchbox_name}_64x300_ptq_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 9C: WIDER MATCHBOXNET @ 64x300 (1D TIME-CHANNEL SEPARABLE)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MatchboxNet-{config['matchbox_name']} WIDER (Model 9c)\n")
            ...
            f.write(f"  C (Channels):           {config['matchbox_C']} (WIDER - 2x baseline)\n")
            ...
            f.write(f"  Memory Target:          <512 KB (wider model may exceed target)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 9c_matchboxnet_wider_{config['matchbox_name']} | Dropout: {config['dropout']} | "
            ...
            f.write(f"9c_matchboxnet_wider,{config['matchbox_name']},{config['dropout']},{config['augmentation_mode']},"
...
# --------------------------------------------------------------
def matchboxnet_subblock(x, channels, kernel_size, dropout_rate, block_id, sub_id):
    """
    MatchboxNet sub-block: 1D time-channel separable convolution.
    ...
    # Depthwise 1D convolution (along time axis)
    ...
    # Pointwise 1D convolution (1x1 to mix channels)
    x = layers.Conv1D(
...
def matchboxnet_residual_block(x, channels, kernel_size, num_subblocks, dropout_rate, block_id):
    """
    MatchboxNet residual block with R sub-blocks.
    ...
    for r in range(num_subblocks):
        x = matchboxnet_subblock(x, channels, kernel_size, dropout_rate, block_id, r)
    ...
    if input_channels != channels:
        ...
        shortcut = layers.BatchNormalization(name=f'block{block_id}_residual_bn')(shortcut)
...
def create_matchboxnet(num_classes, input_shape, B=3, R=2, C=128, dropout=0.2):
    """
    Create WIDER MatchboxNet model for audio classification.

    Architecture: MatchboxNet-BxRxC (WIDER version with C=128)
    ...
    Args:
        ...
        C: Channels per sub-block (default: 128 for WIDER version)
    ...
    # Prologue: Initial conv to expand channels
    x = layers.Conv1D(
        128, kernel_size=11, padding='same',
        use_bias=False, name='prologue_conv'
    )(inputs)
    x = layers.BatchNormalization(name='prologue_bn')(x)
    ...
    for b in range(B):
        ...
        x = matchboxnet_residual_block(x, C, kernel_size, R, dropout, block_id=b)

    # Epilogue: Final convolutions before classification
    ...
    x = layers.BatchNormalization(name='epilogue_bn1')(x)
    ...
    x = layers.Conv1D(
        ...
        use_bias=False, name='epilogue_conv2'
    )(x)
    x = layers.BatchNormalization(name='epilogue_bn2')(x)
    ...
    return keras.Model(inputs, outputs, name=f"MatchboxNet_{B}x{R}x{C}_wider")
...
def main():
    ...
    print(f"\nModel 9c: WIDER MatchboxNet-{config['matchbox_name']} @ 64x300 (1D Time-Channel Separable)")
    print(f"  B (Residual Blocks):    {config['matchbox_B']}")
    print(f"  R (Sub-blocks/Block):   {config['matchbox_R']}")
    print(f"  C (Channels):           {config['matchbox_C']} (WIDER - 2x baseline)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print(f"CREATING WIDER MATCHBOXNET-{config['matchbox_name']} MODEL (1D TIME-CHANNEL SEPARABLE)")
    print(f"{'=' * 70}")
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   9c_matchboxnet_wider_{config['matchbox_name']} (1D Time-Channel)")
    print(f"Configuration:           B={config['matchbox_B']}, R={config['matchbox_R']}, C={config['matchbox_C']} (WIDER)")
    print(f"Dropout:                 {config['dropout']}")
    print(f"Augmentation:            {config['augmentation_mode']}")
