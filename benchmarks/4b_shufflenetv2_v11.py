#!/usr/bin/env python3
"""
Model 4b: ShuffleNetV2 for Mygardenbird Classification
Ultra-efficient architecture optimized for ARM Cortex-M7 deployment
ShuffleNetV2 Key Features:
- Channel Shuffle & Split operations for efficient cross-group communication
- Depthwise separable convolutions with pointwise expansions
- Balanced memory access & direct connections (minimal FLOPs)
- Very small: ~1.0M params → ~280 KB INT8
- Designed for embedded/mobile deployment
Input: 64×300 mel-spectrogram (native, no resize)
Expected: 86-90% accuracy @ ~280 KB INT8
Target: Smallest viable model for Portenta H7
Version 1.1 improvements over baseline:
- Adjusted stride/pooling schedule for 64×300 aspect ratio
- Groups=2 channel shuffle (optimal for ARM NEON/M7)
...
Expected outcome: Validate channel-shuffle efficiency vs SqueezeNet/MobileNet
"""
print("\n" + " ❤️ " * 30 + "\n")
import os
import sys
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 4B: SHUFFLENETV2 @ 64×300 NATIVE (Version 4b)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             ShuffleNetV2 (Model 4b)\n")
            f.write(f"  Input Shape:            {config['input_shape']} (NATIVE, NO RESIZE)\n")
            f.write(f"  Adaptation:             Stride=1 first conv, adjusted pooling for 64x300\n")
            f.write(f"  Channel Groups:         2 (optimized for ARM NEON/M7)\n")
            f.write(f"  Dropout Rate:           {config['dropout']}\n")
            f.write(f"  Activation:             ReLU (ShuffleNet standard)\n")
    ...
    def log_model_info(self, model):
        ...
        with open(self.log_path, 'a') as f:
            ...
            if int8_size_kb > 512: f.write(f"\n⚠ WARNING: Model may exceed 512 KB target\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"Model: 4b_shufflenetv2_v11 | Dropout: {config['dropout']} | Aug: {config['augmentation_mode']} | Seed: {config['random_seed']}\n")
            ...
            f.write(f"4b_shufflenetv2_v11,{config['dropout']},{config['augmentation_mode']},"
                    ...
                    f"{os.path.getsize(os.path.join(config['output_dir'], 'shufflenetv2_int8.tflite')) / 1024:.1f}\n")
...
# --------------------------------------------------------------
# SHUFFLENETV2 ADAPTED FOR 64×300 INPUT
# --------------------------------------------------------------
def channel_shuffle(x, groups):
    """Channel shuffle operation (ShuffleNetV2 core)."""
...
def shufflenet_unit(x, filters, stride, groups, name_prefix):
    """Standard ShuffleNetV2 building block."""
    if stride > 1:
        branch1 = layers.DepthwiseConv2D(3, strides=stride, padding='same',
                                         use_bias=False, name=f'{name_prefix}_dconv_b1')(x)
        branch1 = layers.BatchNormalization(name=f'{name_prefix}_bn_b1')(branch1)
        branch1 = layers.Conv2D(filters // 2, 1, use_bias=False, name=f'{name_prefix}_conv1x1_b1')(branch1)
        branch1 = layers.BatchNormalization(name=f'{name_prefix}_bn_b1_out')(branch1)
        branch1 = layers.Activation('relu', name=f'{name_prefix}_act_b1')(branch1)

        branch2 = layers.Conv2D(filters // 2, 1, use_bias=False, name=f'{name_prefix}_conv1x1_b2')(x)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn_b2')(branch2)
        branch2 = layers.Activation('relu', name=f'{name_prefix}_act_b2')(branch2)
        branch2 = channel_shuffle(branch2, groups)
        branch2 = layers.DepthwiseConv2D(3, strides=stride, padding='same',
                                         use_bias=False, name=f'{name_prefix}_dconv_b2')(branch2)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn_b2_out')(branch2)
        branch2 = layers.Conv2D(filters // 2, 1, use_bias=False, name=f'{name_prefix}_conv1x1_b2_out')(branch2)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn_b2_final')(branch2)
        branch2 = layers.Activation('relu', name=f'{name_prefix}_act_b2_out')(branch2)
        return layers.Concatenate(name=f'{name_prefix}_concat')([branch1, branch2])
    else:
        branch1 = x
        branch2 = layers.Conv2D(filters // 2, 1, use_bias=False, name=f'{name_prefix}_conv1x1')(x)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn')(branch2)
        branch2 = layers.Activation('relu', name=f'{name_prefix}_act')(branch2)
        branch2 = channel_shuffle(branch2, groups)
        branch2 = layers.DepthwiseConv2D(3, strides=1, padding='same',
                                         use_bias=False, name=f'{name_prefix}_dconv')(branch2)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn_out')(branch2)
        branch2 = layers.Conv2D(filters // 2, 1, use_bias=False, name=f'{name_prefix}_conv1x1_out')(branch2)
        branch2 = layers.BatchNormalization(name=f'{name_prefix}_bn_final')(branch2)
        branch2 = layers.Activation('relu', name=f'{name_prefix}_act_out')(branch2)
        return layers.Concatenate(name=f'{name_prefix}_concat')([branch1, branch2])

def create_shufflenetv2_64x300(num_classes, input_shape, dropout=0.2, groups=2):
    inputs = layers.Input(shape=input_shape)
    # Stage 1: Initial conv (stride=1 to preserve 64×300)
    x = layers.Conv2D(24, 3, strides=1, padding='same', use_bias=False, name='conv1')(inputs)
    x = layers.BatchNormalization(name='conv1_bn')(x)
    x = layers.Activation('relu', name='conv1_act')(x)
    x = layers.MaxPooling2D(pool_size=3, strides=2, padding='same', name='pool1')(x)  # → 32×150
    # Stage 2: 116 ch, 4 units → 16×75
    x = shufflenet_unit(x, 116, stride=2, groups=groups, name_prefix='stage2_1')
    for i in range(3):
        x = shufflenet_unit(x, 116, stride=1, groups=groups, name_prefix=f'stage2_{i+2}')
    # Stage 3: 232 ch, 5 units → 8×37
    x = shufflenet_unit(x, 232, stride=2, groups=groups, name_prefix='stage3_1')
    for i in range(4):
        x = shufflenet_unit(x, 232, stride=1, groups=groups, name_prefix=f'stage3_{i+2}')
    # Stage 4: 464 ch, 3 units → 4×18
    x = shufflenet_unit(x, 464, stride=2, groups=groups, name_prefix='stage4_1')
    for i in range(2):
        x = shufflenet_unit(x, 464, stride=1, groups=groups, name_prefix=f'stage4_{i+2}')
    # Final conv & dropout
    x = layers.Conv2D(1024, 1, use_bias=False, name='conv_final')(x)
    x = layers.BatchNormalization(name='conv_final_bn')(x)
    x = layers.Activation('relu', name='conv_final_act')(x)
    x = layers.Dropout(dropout, name='dropout')(x)
    x = layers.GlobalAveragePooling2D(name='global_pool')(x)
    outputs = layers.Dense(num_classes, activation='softmax', name='softmax')(x)
    return keras.Model(inputs, outputs, name="ShuffleNetV2_64x300")
...
def main():
    ...
    print(f"\nModel 4b: ShuffleNetV2 @ 64×300 (NATIVE)")
    print(f"  Hypothesis: Channel-shuffle > Fire/Mobile blocks for spectrograms")
    ...
    print(f"\n{'=' * 70}\nCREATING SHUFFLENETV2 MODEL (64×300 NATIVE)\n{'=' * 70}")
    model = create_shufflenetv2_64x300(num_classes, config['input_shape'], config['dropout'])
    model.summary()
    ...
    fp32_path = os.path.join(config['output_dir'], 'shufflenetv2_fp32.keras')
    ...
    int8_path = os.path.join(config['output_dir'], 'shufflenetv2_int8.tflite')
    ...
    total_time = time.time() - script_start
    print(f"\n{'=' * 70}\nFINAL RESULTS\n{'=' * 70}")
    print(f"Model:                   4b_shufflenetv2_v11 (64×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
