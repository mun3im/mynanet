#!/usr/bin/env python3
"""
Model 4c: ShuffleNetV2-Compact for Mygardenbird Classification
Scaled-down ShuffleNetV2 targeting ≤512 KB INT8 for Portenta H7 deployment.
Architecture: channels 48/96/192, final conv 512, stem 16ch — ~220 KB INT8.
Same channel-shuffle efficiency as 4b at H7-deployable size.
Input: 64×300 mel-spectrogram (native, no resize)
Expected: ~90% accuracy @ ~220 KB INT8
Target: Portenta H7 flash ≤512 KB, MCU-compatible ops only
"""
print("\n" + " ❤️ " * 30 + "\n")
import os
import sys
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 4C: SHUFFLENETV2-COMPACT @ 64×300 NATIVE (Version 4c)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             ShuffleNetV2-Compact (Model 4c)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"Model: 4c_shufflenetv2_compact | Dropout: {config['dropout']} | Aug: {config['augmentation_mode']} | Seed: {config['random_seed']}\n")
            ...
            f.write(f"4c_shufflenetv2_compact,{config['dropout']},{config['augmentation_mode']},"
                    ...
                    f"{os.path.getsize(os.path.join(config['output_dir'], 'shufflenetv2_compact_int8.tflite')) / 1024:.1f}\n")
...
def create_shufflenetv2_64x300(num_classes, input_shape, dropout=0.2, groups=2):
    ...
    # Stage 1: stem 16ch, stride=1 to preserve 64×300 → pool → 32×150
    x = layers.Conv2D(16, 3, strides=1, padding='same', use_bias=False, name='conv1')(inputs)
    ...
    # Stage 2: 48 ch, 4 units → 16×75
    x = shufflenet_unit(x, 48, stride=2, groups=groups, name_prefix='stage2_1')
    for i in range(3):
        x = shufflenet_unit(x, 48, stride=1, groups=groups, name_prefix=f'stage2_{i+2}')
    # Stage 3: 96 ch, 5 units → 8×38
    x = shufflenet_unit(x, 96, stride=2, groups=groups, name_prefix='stage3_1')
    for i in range(4):
        x = shufflenet_unit(x, 96, stride=1, groups=groups, name_prefix=f'stage3_{i+2}')
    # Stage 4: 192 ch, 3 units → 4×19
    x = shufflenet_unit(x, 192, stride=2, groups=groups, name_prefix='stage4_1')
    for i in range(2):
        x = shufflenet_unit(x, 192, stride=1, groups=groups, name_prefix=f'stage4_{i+2}')
    # Final conv 384ch & dropout (keeps INT8 under 512 KB)
    x = layers.Conv2D(384, 1, use_bias=False, name='conv_final')(x)
    ...
    return keras.Model(inputs, outputs, name="ShuffleNetV2_Compact_64x300")
...
def main():
    ...
    print(f"\nModel 4c: ShuffleNetV2-Compact @ 64×300 (NATIVE)")
    print(f"  Hypothesis: Full ShuffleNetV2 accuracy at H7-deployable size (≤512 KB)")
    ...
    print(f"\n{'=' * 70}\nCREATING SHUFFLENETV2-COMPACT MODEL (64×300 NATIVE)\n{'=' * 70}")
    ...
    fp32_path = os.path.join(config['output_dir'], 'shufflenetv2_compact_fp32.keras')
    ...
    int8_path = os.path.join(config['output_dir'], 'shufflenetv2_compact_int8.tflite')
    ...
    total_time = time.time() - script_start
    print(f"\n{'=' * 70}\nFINAL RESULTS\n{'=' * 70}")
    print(f"Model:                   4c_shufflenetv2_compact (64×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
