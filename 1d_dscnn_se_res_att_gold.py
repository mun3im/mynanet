#!/usr/bin/env python3
"""
DS-CNN + Squeeze-Excitation + Residual + Attention -> Model 1d
...
def get_config():
    ...
    output_dir_name = (
        f"results/1d_dscnn_se_res_att_"
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             DS-CNN + SE + Residual + Attention (Model 1d)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"1d_dscnn_se_res_att,{config['dropout']},{config['augmentation_mode']},"
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  DS-CNN + SE + Residual + Attention (Model 1d)\n")
...
def create_se_res_att(num_classes, input_shape, dropout=0.2):
    """
    DS-CNN + SE + Residual + Lightweight MHSA (Model 1d)
    Identical to create_dscnn_se_res but adds a lightweight MultiHeadAttention
    after the last DS block and before GlobalAveragePooling.
    Estimated added params: ~13K → total ~310 KB INT8.
    """
    ...
    h, w, c = x.shape[1], x.shape[2], x.shape[3]  # h=4, w=18, c=512
    x = layers.Reshape((h * w, c), name='reshape_for_attention')(x)  # (72, 512)

    # Project to lower dimension to reduce attention cost
    x_proj = layers.Dense(64, activation='relu', name='attention_proj')(x)  # (72, 64)

    # Multi-head self-attention (2 heads, key_dim=32 → 64 total)
    ...
    # Optional: Add & norm (skip for minimal overhead)
    ...
    return keras.Model(inputs, outputs, name="MynaNet_DSCNN_SE_Res_Att")
...
def main():
    ...
    print(f"  Model: DS-CNN + SE + Residual + Att (1d)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING DS-CNN + SE + RESIDUAL MODEL + ATTENTION (Model 1d)")
    print(f"{'=' * 70}")
    model = create_se_res_att(num_classes, config['input_shape'],
                                config['dropout'])
    model.summary()
