#!/usr/bin/env python3
"""
Model 9a - MatchboxNet for Mygardenbird Classification
...
Architecture: MatchboxNet-3x2x64 (NVIDIA, 2020)
...
Key features:
- Prologue conv (128 ch, k=11) → 3 residual blocks → Epilogue convs
- Sub-block: DepthwiseConv1D → BN → ReLU → Dropout → PointwiseConv → BN → ReLU
- Large kernels capture temporal patterns in bird calls
...
def get_config():
    ...
    # MatchboxNet naming: 9a_matchboxnet_BxRxC
    matchbox_name = f"{args.matchbox_B}x{args.matchbox_R}x{args.matchbox_C}"

    output_dir_name = (
        f"results/9a_matchboxnet_{matchbox_name}_64x300_ptq_"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 9A: MATCHBOXNET @ 64x300 (1D TIME-CHANNEL SEPARABLE)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MatchboxNet-{config['matchbox_name']} (Model 9a)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 9a_matchboxnet_{config['matchbox_name']} | Dropout: {config['dropout']} | "
            ...
            f.write(f"9a_matchboxnet,{config['matchbox_name']},{config['dropout']},{config['augmentation_mode']},"
...
def create_matchboxnet(num_classes, input_shape, B=3, R=2, C=64, dropout=0.2):
    """
    Create MatchboxNet model for audio classification.
    ...
    return keras.Model(inputs, outputs, name=f"MatchboxNet_{B}x{R}x{C}")
...
def main():
    ...
    print(f"\nModel 9a: MatchboxNet-{config['matchbox_name']} @ 64x300 (1D Time-Channel Separable)")
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   9a_matchboxnet_{config['matchbox_name']} (1D Time-Channel)")
