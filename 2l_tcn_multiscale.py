#!/usr/bin/env python3
"""
Model 4p - Multi-Scale TCN: TCN with Parallel Multi-Scale Branches
- Based on 4n combined variant
...
- SE blocks: Add adaptive channel recalibration
- Goal: 90-92% accuracy with minimal size increase
- Expected size: ~920-950 KB (vs 4n's ~900 KB)
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4p - Multi-Scale TCN)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Temporal Convolutional Network (TCN) - MULTI-SCALE VARIANT\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    """
    Create Multi-Scale TCN with parallel branches for different temporal receptive fields.

    Architecture:
    - 3 parallel TCN branches with kernels [3, 5, 7]
    - Each branch: 64 channels (total 192 after concatenation)
    - Concatenate outputs for multi-scale feature fusion
    - SE block on concatenated features for channel attention
    - Expected improvement: +2-4% over single-scale TCN
    """
    inputs = layers.Input(shape=input_shape)
    x_base = layers.Permute((2, 1, 3))(inputs)
    x_base = layers.Reshape((input_shape[1], input_shape[0]))(x_base)
    x_base = layers.Dense(channels, activation='relu6')(x_base)
    x_base = layers.Dropout(dropout * 0.5)(x_base)

    # Create 3 parallel branches with different kernel sizes
    branches = []
    for kernel_size in [3, 5, 7]:
        x = x_base
        branch_skips = []

        # 2 stacks of dilated convolutions per branch
        for _ in range(2):
            for d in [1, 2, 4, 8, 16, 32]:
                res = x
                x = layers.Conv1D(channels, kernel_size, dilation_rate=d, padding='causal', activation='relu6')(x)
                x = layers.Dropout(dropout)(x)
                x = layers.Conv1D(channels, kernel_size, dilation_rate=d, padding='causal')(x)
                ...
                x = layers.Dropout(dropout)(x)
                branch_skips.append(layers.Conv1D(channels, 1)(x))

        # Sum skip connections within branch
        if branch_skips:
            x = layers.Add()(branch_skips)
            x = layers.Activation('relu6')(x)

        branches.append(x)

    # Concatenate multi-scale features
    x = layers.Concatenate()(branches)  # Now has 192 channels (64×3)

    # Apply SE block to multi-scale features
    ...
    return keras.Model(inputs, outputs, name="Mygardenbird_MultiScale_TCN")
