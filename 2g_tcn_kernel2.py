...
- Hypothesis: Smaller kernel = faster but may need more layers
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4e - TCN Kernel2)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Temporal Convolutional Network (TCN) - KERNEL2 VARIANT\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    ...
    for _ in range(2):
        for d in [1, 2, 4, 8, 16, 32]:
            res = x
            x = layers.Conv1D(channels, 2, dilation_rate=d, padding='causal', activation='relu6')(x)
            x = layers.Dropout(dropout)(x)
            x = layers.Conv1D(channels, 2, dilation_rate=d, padding='causal')(x)
...
def main():
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING TCN MODEL (kernel_size=2)")
    print(f"{'=' * 70}")
