#!/usr/bin/env python3
"""
MBV3-Matchbox - Model 1o
1j (MynaNet) shrunk with MatchboxNet features (Majumdar & Ginsburg, Interspeech 2020)
Post-Training Quantization (PTQ) → Cortex-M7 deployment
Fixed spectrogram shape: 64x300 (10ms per frame)

MatchboxNet features ported onto 1j (blocks 1-3 + SE identical to 1j):
  1. Fully convolutional epilogue — no Dense bottleneck
     - MatchboxNet's head is Conv2/Conv3/Conv4 (1×1 convs) straight into softmax
     - Removes 1j's Conv2D(320,1×1)+Dense(128) head: −74K params (38% of 1j)
     - GAP → Dense(n_classes) is the exact equivalent of their 1×1 Conv4 + pool
  2. Dilated large-time-kernel epilogue conv (their Conv2: k=29, dilation=2)
     - DepthwiseConv2D(1×17, time-only) before the 1×1 projection
     - Buys temporal receptive field (~17 frames at block-4 resolution ≈ 2.7 s
       of input) for <1K params, replacing width with receptive field
  3. Narrow, near-uniform channels (their C=64 blocks, 128 epilogue)
     - Block 4 capped at 64 channels instead of doubling 48→96
     - Epilogue 1×1 projects to 128 channels like MatchboxNet Conv3
  4. B×R×C-style scaling knobs: --block4_filters / --epilogue_filters

Model Architecture:
  Conv2D(32, 3×3) + BN + ReLU6                          # Stem       (= 1j)
  InvRes-SE(t=1, 32→16,  dw=3×3) + MaxPool2D            # Block 1    (= 1j)
  InvRes-SE(t=6, 16→24,  dw=3×3) + MaxPool2D            # Block 2    (= 1j)
  InvRes-SE(t=6, 24→48,  dw=5×5) + MaxPool2D            # Block 3    (= 1j)
  InvRes-SE(t=6, 48→64,  dw=5×5) + MaxPool2D            # Block 4    ← 96→64
  DWConv(1×17, time-only) + BN + ReLU6                # Matchbox 'Conv2'
  Conv2D(128, 1×1) + BN + ReLU6                         # Matchbox 'Conv3'
  GlobalAveragePooling2D + Dropout
  Dense(n_classes, softmax)                             # Matchbox 'Conv4'

~120K params (−38% vs 1j's 193,396), est. ~190 KB INT8 vs 267 KB.
All ops H7-safe: Conv2D, DepthwiseConv2D, BN, ReLU6, Add, Mul — no BATCH_MATMUL.
Target: ≥94% INT8 accuracy at ≤200 KB, MCU-deployable (Portenta H7)
"""
...
def get_config():
    ...
    # MatchboxNet-style C-scaling knobs (their Table 5 scales channels the same way)
    parser.add_argument("--block4_filters", type=int, default=64,
                        help="Block 4 output channels (1j uses 96; default 64)")
    parser.add_argument("--epilogue_filters", type=int, default=128,
                        help="Epilogue 1x1 conv channels, MatchboxNet Conv3 analog (default: 128)")
    ...
    # Encode non-default C-scaling in the folder name so scaled runs don't collide
    scale_suffix = ""
    if args.block4_filters != 64 or args.epilogue_filters != 128:
        scale_suffix = f"b4{args.block4_filters}_ep{args.epilogue_filters}_"
    ...
    config = {
        ...
        'model_type': 'mbv3_matchbox',
        'block4_filters': args.block4_filters,
        'epilogue_filters': args.epilogue_filters,
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 1O: MBV3-MATCHBOX (1j shrunk with MatchboxNet epilogue, narrow block 4)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MBV3-Matchbox (Model 1o = 1j + MatchboxNet shrink)\n")
            f.write(f"  Architecture:           Stem32→InvRes16→24→48→{config['block4_filters']} + Matchbox epilogue\n")
            f.write(f"  Dropout Rate:           {config['dropout']}\n")
            f.write(f"  Input Shape:            {config['input_shape']}\n")
            f.write(f"  Conv Blocks:            4 InvRes-SE blocks + 1 stem conv\n")
            f.write(f"  Block 4 Channels:       {config['block4_filters']} (1j: 96)\n")
            f.write(f"  Epilogue:               DW(1×17 time-only) + Conv1×1({config['epilogue_filters']}) — MatchboxNet Conv2/Conv3\n")
            f.write(f"  Pooling:                MaxPool2D (2×2) after each InvRes block\n")
            f.write(f"  Activation:             ReLU6 (quantization-friendly)\n")
            f.write(f"  Global Pooling:         GlobalAveragePooling2D\n")
            f.write(f"  Head:                   GAP → softmax (no Dense bottleneck, MatchboxNet FCN head)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"1o_mbv3_matchbox,{config['dropout']},{config['augmentation_mode']},"
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  MBV3-Matchbox (Model 1o = 1j + MatchboxNet shrink)\n")
            f.write(f"  - 4 InvRes-SE blocks: 16→24→48→{config['block4_filters']} filters (block 4 capped)\n")
            f.write(f"  - MatchboxNet epilogue: DW(1×17 time-only) + Conv1×1({config['epilogue_filters']})\n")
            f.write(f"  - Fully convolutional head: GAP → softmax (no Dense bottleneck)\n")
...
# --------------------------------------------------------------
# INVERTED RESIDUAL + SE BLOCKS (Model 1o — MBV3-Matchbox)
# --------------------------------------------------------------
...
def inverted_residual_se(x, filters, kernel_size=(3, 3), strides=(1, 1),
    ...
    Structure (MBV3-SE):
      ...
      k×k DepthwiseConv (3×3 or 5×5)  → BN → ReLU6       # spatial filter
      SE block (hard-sigmoid excitation)                   # squeeze-excite
      1×1 Conv (project: in*t → out)   → BN               # pointwise project (no activation)
      Add shortcut if in==out and stride==1                # residual

    5×5 depthwise (MBV3 feature, blocks 3-4): wider spectral/temporal receptive field.
    Hard-sigmoid SE: better INT8 quantization than sigmoid.
    All ops: Conv2D, DepthwiseConv2D, BatchNorm, ReLU6, Add, Mul — all H7 compatible.
    """
...
def create_mbv3_matchbox(num_classes, input_shape, dropout=0.2,
                         block4_filters=64, epilogue_filters=128):
    """
    MBV3-Matchbox (Model 1o) — 1j shrunk with MatchboxNet features.

    Architecture:
      Input: (n_mels, 300, 1) mel-spectrogram
      Conv2D(32, 3×3) + BN + ReLU6                          # Stem       (= 1j)
      InvRes-SE(t=1, 32→16,  dw=3×3) + MaxPool2D + Dropout # Block 1    (= 1j)
      InvRes-SE(t=6, 16→24,  dw=3×3) + MaxPool2D + Dropout # Block 2    (= 1j)
      InvRes-SE(t=6, 24→48,  dw=5×5) + MaxPool2D + Dropout # Block 3    (= 1j)
      InvRes-SE(t=6, 48→64,  dw=5×5) + MaxPool2D + Dropout # Block 4    ← 96→64
      DWConv(1×17, time-only) + BN + ReLU6                # Matchbox 'Conv2'
      Conv2D(128, 1×1) + BN + ReLU6                         # Matchbox 'Conv3'
      GlobalAveragePooling2D + Dropout
      Dense(n_classes, softmax)                             # Matchbox 'Conv4'

    Changes from 1j (each a MatchboxNet feature, Majumdar & Ginsburg 2020):
    - Fully convolutional epilogue: drops 1j's Conv2D(320,1×1) + Dense(128)
      bottleneck (~74K params). GAP→Dense(n_classes) ≡ their 1×1 Conv4 + pool.
    - Large time-only depthwise conv before the projection (their Conv2,
      kernel 29 dilation 2): temporal receptive field for ~1K params.
    - Narrow near-uniform channels (their C=64): block 4 stays at 64 instead
      of doubling to 96; epilogue projects to 128 like their Conv3.
    - block4_filters / epilogue_filters expose MatchboxNet-style C-scaling.

    ~120K params (1j: 193,396). Estimated ~190 KB INT8 (1j: 267 KB).
    All ops H7-compatible: Conv2D, DepthwiseConv2D, BN, ReLU6, Add, Mul, Lambda(relu6).
    """
    ...
    # Block 3: t=6, 24→48, dw=5×5 (MBV3 wider receptive field)
    ...
    # Block 4: t=6, 48→64, dw=5×5 — capped at 64 (MatchboxNet uniform width; 1j doubles to 96)
    x = inverted_residual_se(x, block4_filters, kernel_size=(5, 5), block_id=4,
                             expand_ratio=6, se_reduction=16)
    x = layers.MaxPooling2D((2, 2), name='pool4')(x)
    x = layers.Dropout(dropout, name='drop4')(x)

    # MatchboxNet epilogue (their Conv2/Conv3/Conv4) — replaces 1j's
    # Conv2D(320,1×1) + Dense(128) head. Spatial dims 4×18 here; the 1×17
    # time-only depthwise spans ≈ 2.7 s of input — same receptive field as
    # MatchboxNet's dilated Conv2 (k=9, d=2) but expressed without dilation:
    # TFLite lowers dilated DW convs to SPACE_TO_BATCH_ND/BATCH_TO_SPACE_ND,
    # which are not safe for TFLite Micro on the H7.
    x = layers.DepthwiseConv2D((1, 17), padding='same',
    ...
    x = layers.Conv2D(epilogue_filters, (1, 1), padding='same', use_bias=False,
                      name='epilogue_pw')(x)
    x = layers.BatchNormalization(name='epilogue_pw_bn')(x)
    x = layers.ReLU(6., name='epilogue_pw_relu')(x)

    # GAP straight into softmax — no Dense bottleneck (MatchboxNet Conv4 ≡ 1×1 conv)
    x = layers.GlobalAveragePooling2D(name='global_pool')(x)
    x = layers.Dropout(dropout, name='fc_drop')(x)
    outputs = layers.Dense(num_classes, activation='softmax', name='output')(x)

    return keras.Model(inputs, outputs, name="MynaNet_MBV3_Matchbox")
...
def main():
    ...
    print(f"  Model: MBV3-Matchbox (1o) — 1j + MatchboxNet FCN epilogue, block4={config['block4_filters']}, epilogue={config['epilogue_filters']}")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING MBV3-MATCHBOX MODEL (Model 1o) — 1j + MatchboxNet epilogue, narrow block 4")
    print(f"{'=' * 70}")
    model = create_mbv3_matchbox(num_classes, config['input_shape'],
                                 config['dropout'],
                                 block4_filters=config['block4_filters'],
                                 epilogue_filters=config['epilogue_filters'])
    model.summary()
