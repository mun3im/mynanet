...
New features over 1j (MBV3-SE):
  1. Stacked residual repeat at block 3: two InvRes-SE at 48ch back-to-back.
     - Second block (3b) fires residual Add (dims match: 48→48, stride=1).
     - First stacked residual in this family — adds representational depth for free.
  2. Hard-swish activation in blocks 3–5 (expand + depthwise steps).
     - hard_swish(x) = x * relu6(x+3)/6
     - MBV3-Large activation; richer than ReLU6 at no extra parameters.
     - Blocks 1–2 keep ReLU6 (small feature maps don't benefit).
  3. Fifth inverted-residual block (96→96ch, 5×5 dw, hard-swish) before last conv.
     - Residual Add fires (96→96, stride=1) — second stacked residual in the model.
     - Adds depth without bloating channels; last conv stays at 320ch.
     - Total: ~488 KB INT8 — fits H7 512 KB flash.
...
Model Architecture:
  ...
  InvRes-SE(t=6, 24→48,  dw=5×5, h-swish) + (no pool)     # Block 3a  ← new: h-swish
  InvRes-SE(t=6, 48→48,  dw=5×5, h-swish) + MaxPool2D     # Block 3b  ← new: stacked+residual
  InvRes-SE(t=6, 48→96,  dw=5×5, h-swish) + MaxPool2D     # Block 4   ← new: h-swish
  InvRes-SE(t=6, 96→96,  dw=5×5, h-swish) + MaxPool2D     # Block 5   ← new: stacked residual fires
  Conv2D(320, 1×1) + BN + ReLU6                            # Last expansion
  GlobalAveragePooling2D
  Dense(128, relu6) + Dropout + Dense(n_classes, softmax)

Target: >95% INT8 accuracy, <512KB model size, MCU-deployable (Portenta H7)
"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MODEL 1K: MBV3-SE v2 (stacked residual, hard-swish, 5th block, 960ch last conv)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             MBV3-SE v2 Inverted Residual (Model 1k)\n")
            f.write(f"  Architecture:           Stem→Blk1(16)→Blk2(24)→Blk3a/b(48)→Blk4(96)→Blk5(160)→FC960\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"1k_mbv3_se_v2,{config['dropout']},{config['augmentation_mode']},"
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  MBV3-SE v2 Inverted Residual (Model 1k)\n")
            f.write(f"  - 5 InvRes-SE blocks: 16→24→48(×2)→96→160ch\n")
            f.write(f"  - Hard-swish in blocks 3a/3b/4/5; ReLU6 in blocks 1/2\n")
            f.write(f"  - Stacked residual at block 3 (3b fires Add — dims match)\n")
            f.write(f"  - Last conv: 960ch (MBV3-Large); GAP + Dense(128) head\n")
...
# --------------------------------------------------------------
# INVERTED RESIDUAL + SE BLOCKS (Model 1k — MobileNetV3-SE v2)
# --------------------------------------------------------------
def se_block(x, filters, reduction=16, block_id=0, suffix=''):
    """
    Squeeze-and-Excitation block with MBV3 hard-sigmoid excitation.
    Hard-sigmoid (MBV3): relu6(x+3)/6 — decomposes to Add+ReLU6+Mul, H7-safe.
    """
    prefix = f'block{block_id}{suffix}_se_'
...
def _hard_swish(x, name):
    """hard_swish(x) = x * relu6(x+3)/6 — MBV3-Large activation."""
    scale = layers.Lambda(
        lambda t: tf.nn.relu6(t + 3.0) * (1.0 / 6.0),
        name=name + '_hs_gate'
    )(x)
    return layers.Multiply(name=name + '_hs_mul')([x, scale])


def inverted_residual_se(x, filters, kernel_size=(3, 3), strides=(1, 1),
                         expand_ratio=6, block_id=0, se_reduction=16,
                         activation='relu6', suffix=''):
    """
    MobileNetV3-style Inverted Residual block with SE + hard-sigmoid.

    activation: 'relu6' (blocks 1-2) or 'hard_swish' (blocks 3+)
    suffix: appended to block_id for stacked blocks (e.g. suffix='b' for block 3b)

    All ops: Conv2D, DepthwiseConv2D, BatchNorm, ReLU6, Add, Mul — H7 compatible.
    """
    prefix = f'block{block_id}{suffix}_'
    ...
    x = se_block(x, expanded, reduction=max(1, expand_ratio * se_reduction // 16),
                 block_id=block_id, suffix=suffix)
...
def create_mbv3_se_v2(num_classes, input_shape, dropout=0.2):
    """
    MobileNetV3-SE v2 (Model 1k).

    New over 1j:
    1. Stacked residual at block 3: 3a (24→48) + 3b (48→48, residual Add fires)
    2. Hard-swish in expand+depthwise of blocks 3a, 3b, 4, 5
    3. Block 5: 96→160ch, 5×5 dw, hard-swish; last conv: 320→960 (MBV3-Large)

    Architecture:
      Conv2D(32, 3×3) + BN + ReLU6                              # Stem
      InvRes-SE(t=1, 32→16,  dw=3×3, ReLU6)  + MaxPool2D       # Block 1
      InvRes-SE(t=6, 16→24,  dw=3×3, ReLU6)  + MaxPool2D       # Block 2
      InvRes-SE(t=6, 24→48,  dw=5×5, h-swish)                  # Block 3a (no pool)
      InvRes-SE(t=6, 48→48,  dw=5×5, h-swish) + MaxPool2D      # Block 3b (residual!)
      InvRes-SE(t=6, 48→96,  dw=5×5, h-swish) + MaxPool2D      # Block 4
      InvRes-SE(t=6, 96→96,  dw=5×5, h-swish) + MaxPool2D      # Block 5 (new; residual fires)
      Conv2D(320, 1×1) + BN + ReLU6                             # Last expansion
      GlobalAveragePooling2D
      Dense(128, relu6) + BN + Dropout + Dense(n_classes, softmax)
    """
    ...
    # Block 3a: t=6, 24→48, dw=5×5, hard-swish (no pool — 3b follows)
    x = inverted_residual_se(x, 48, kernel_size=(5, 5), block_id=3, expand_ratio=6,
                             se_reduction=16, activation='hard_swish', suffix='a')

    # Block 3b: t=6, 48→48, dw=5×5, hard-swish — residual Add fires (dims match)
    x = inverted_residual_se(x, 48, kernel_size=(5, 5), block_id=3, expand_ratio=6,
                             se_reduction=16, activation='hard_swish', suffix='b')
    ...
    # Block 5: t=6, 96→96, dw=5×5, hard-swish (new depth; residual Add fires — dims match)
    x = inverted_residual_se(x, 96, kernel_size=(5, 5), block_id=5, expand_ratio=6,
                             se_reduction=16, activation='hard_swish')
    ...
    return keras.Model(inputs, outputs, name="MynaNet_MBV3_SE_V2")
...
def main():
    ...
    print(f"{'=' * 70}")
    model = create_mbv3_se_v2(num_classes, config['input_shape'],
                           config['dropout'])
    model.summary()
