#!/usr/bin/env python3
"""
Depthwise Separable CNN - Model 2 (Improved Baseline)
...
Model Architecture: Depthwise Separable CNN (~150K parameters, <512KB INT8)
  Conv2D(32, 3×3) → BN → ReLU6                      # Initial feature extraction
  DS-Conv(64, 3×3) → BN → ReLU6 → MaxPool2D(2×2)    # DS Block 1
  DS-Conv(128, 3×3) → BN → ReLU6 → MaxPool2D(2×2)   # DS Block 2
  DS-Conv(256, 3×3) → BN → ReLU6 → MaxPool2D(2×2)   # DS Block 3
  DS-Conv(512, 3×3) → BN → ReLU6 → MaxPool2D(2×2)   # DS Block 4 (NEW)
  GlobalAveragePooling2D
  Dense(128) → Dropout → Dense(10)

DS-Conv = DepthwiseConv2D(3×3) + Conv2D(1×1) (pointwise)
  - ~8-9x fewer parameters than standard Conv2D
  - Same receptive field, better parameter efficiency
  - Proven in MobileNet, EfficientNet architectures

Parameter Efficiency:
  Standard Conv(128→256, 3×3): 128×256×3×3 = 294,912 params
  DS-Conv(128→256, 3×3): 128×3×3 + 128×256 = 1,152 + 32,768 = 33,920 params
  Savings: 8.7x fewer parameters!

Purpose: Improved baseline with deeper network but efficient params
Target: >90% INT8 accuracy, <512KB model size
"""
...
class TrainingLogger:
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("\nModel Architecture:\n")
            f.write(f"  Model Type:             Depthwise Separable CNN (Model 2)\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"\nModel Architecture:\n")
            f.write(f"  Depthwise Separable CNN (Model 2)\n")
...
# --------------------------------------------------------------
# DEPTHWISE SEPARABLE CNN MODEL (Model 2 - Improved Baseline)
# --------------------------------------------------------------
def ds_conv_block(x, filters, kernel_size=(3, 3), strides=(1, 1), block_id=0):
    """
    Depthwise Separable Convolution Block.

    DS-Conv = DepthwiseConv2D + BatchNorm + ReLU6 + Conv2D(1×1) + BatchNorm + ReLU6

    This is ~8-9x more parameter efficient than standard Conv2D:
    - Standard Conv(C_in→C_out, 3×3): C_in × C_out × 3 × 3 params
    - DS-Conv(C_in→C_out, 3×3): C_in × 3 × 3 + C_in × C_out params

    Example: 128→256 channels
    - Standard: 128×256×9 = 294,912 params
    - DS-Conv: 128×9 + 128×256 = 1,152 + 32,768 = 33,920 params (8.7x fewer!)
    """
...
def create_dscnn(num_classes, input_shape, dropout=0.2):
    """
    Create Depthwise Separable CNN for seabird classification (Model 2).

    Architecture:
      Input: (64, 300, 1) mel-spectrogram
      Conv2D(32, 3×3) + BN + ReLU6                    # Initial feature extraction
      DS-Conv(64) + MaxPool2D(2×2) + Dropout          # DS Block 1: 64×300 → 32×150
      DS-Conv(128) + MaxPool2D(2×2) + Dropout         # DS Block 2: 32×150 → 16×75
      DS-Conv(256) + MaxPool2D(2×2) + Dropout         # DS Block 3: 16×75 → 8×37
      DS-Conv(512) + MaxPool2D(2×2) + Dropout         # DS Block 4: 8×37 → 4×18
      GlobalAveragePooling2D
      Dense(128, relu6) + Dropout + Dense(10, softmax)

    Parameters: ~150K (target: <512 KB INT8)
    - 4 DS blocks instead of 3 standard conv blocks
    - Deeper network with efficient parameters
    - Better feature hierarchy for audio classification

    Returns:
        Keras Model with ~150K params, suitable for Cortex-M7 deployment
    """
    inputs = layers.Input(shape=input_shape, name='input')

    # Initial conv to expand channels (standard conv, small)
    x = layers.Conv2D(32, (3, 3), padding='same', use_bias=False, name='initial_conv')(inputs)
    x = layers.BatchNormalization(name='initial_bn')(x)
    x = layers.ReLU(6., name='initial_relu')(x)

    # DS Block 1: 32→64 channels (64×300 → 32×150)
    x = ds_conv_block(x, 64, block_id=1)
    x = layers.MaxPooling2D((2, 2), name='pool1')(x)
    x = layers.Dropout(dropout * 0.5, name='drop1')(x)

    # DS Block 2: 64→128 channels (32×150 → 16×75)
    x = ds_conv_block(x, 128, block_id=2)
    x = layers.MaxPooling2D((2, 2), name='pool2')(x)
    x = layers.Dropout(dropout * 0.5, name='drop2')(x)

    # DS Block 3: 128→256 channels (16×75 → 8×37)
    x = ds_conv_block(x, 256, block_id=3)
    x = layers.MaxPooling2D((2, 2), name='pool3')(x)
    x = layers.Dropout(dropout * 0.75, name='drop3')(x)

    # DS Block 4: 256→512 channels (8×37 → 4×18) - NEW!
    x = ds_conv_block(x, 512, block_id=4)
    ...
    return keras.Model(inputs, outputs, name="Seabird_DSCNN")
...
def main():
    ...
    print(f"{'=' * 70}")
    model = create_dscnn(num_classes, config['input_shape'],
                         config['dropout'])
    model.summary()
