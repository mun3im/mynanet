#!/usr/bin/env python3
"""
Stage9e: Seabird Champion on MyGardenBird Dataset
Best Seabird Model (1e: 96.11% @ 64x300) tested on MyGardenBird 80/10/10 split
Post-Training Quantization (PTQ) → Cortex-M7 deployment
Spectrogram: 64x300 (10ms per frame, 64 mel bins)
Data Split: 80/10/10 CSV-based (train/val/test) from MyGardenBird

Model Architecture: DS-CNN with SE attention and Residual in ALL blocks (~886 KB INT8)
...
def get_config():
    ...
    parser.add_argument("--splits_csv", type=str, required=True,
                        help="Path to splits CSV (e.g., splits_mip_80_10_10.csv)")
