...
Fixed spectrogram shape: 64x300 (10ms per frame) or 80x300
...
if platform.system() == "Darwin" and platform.processor() == "arm":
    ...
    print("Using AdamW (weight decay regularization)")
...
def get_config():
    ...
    parser.add_argument("--n_mels", type=int, default=DEFAULT_N_MELS, choices=[64, 80],
                        help="Number of mel frequency bins (64 or 80, default: 64)")

    args = parser.parse_args()

    # Set n_mels from argument
    n_mels = args.n_mels
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYGARDENBIRD TCN TRAINING REPORT (Model 4a - TCN Baseline)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_hyperparameters(self, config):
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write(f"  Mel Bins (N_MELS):      {config.get('n_mels', DEFAULT_N_MELS)}\n")
            f.write(f"  Max Frequency (FMAX):   {FMAX} Hz\n")
            f.write(f"  Time Frames:            {TIME_FRAMES} (FIXED)\n")
            f.write(f"  Spectrogram Shape:      {config.get('n_mels', DEFAULT_N_MELS)}x{TIME_FRAMES}\n")
...
def create_tcn(num_classes, input_shape, dropout=0.2, channels=64):
    """Create TCN model for mygardenbird classification."""
...
def main():
    ...
    # Compute global stats (train files only, no leakage)
    ...
    # Compile for warmup
    model.compile(
        optimizer=Optimizer(learning_rate=config['warmup_lr']),
        loss=loss_function,
        metrics=['accuracy']
    )
    ...
    model.compile(
        optimizer=Optimizer(learning_rate=config['finetune_lr']),
        loss=loss_function,
        metrics=['accuracy']
    )
