#!/usr/bin/env python3
"""
Ablation Study 2d: MobileNetV3Small for Mygardenbird Classification
...
Hypothesis: Standard MobileNetV3Small uses 224×224 input which is SUBOPTIMAL
for spectrograms. We adapt the architecture to work directly with 48×300 input
without resizing, preserving temporal and frequency resolution.

Architecture: MobileNetV3Small adapted for 48×300
- Remove first stride-2 conv (preserve resolution)
- Adjust spatial reduction to account for 48×300 shape
...
Version 2d: Based on 7d pipeline with MobileNetV3Small
...
DEFAULT_SPECTROGRAM_DIR = "/Volumes/Evo/precompute/mygardenbird_spectrograms_48x300"
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("ABLATION 2D: MOBILENETV3SMALL @ 48×300 NATIVE (Version 2d)\n")
            f.write("=" * 80 + "\n")
    ...
    def log_final_results(self, fp32_acc, int8_acc, model_sizes,
        ...
        with open(self.log_path, 'a') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write(f"Model: 2d_mobilenetv3_48x300 | Dropout: {config['dropout']} | "
...
def create_mobilenetv3_small_48x300(num_classes, input_shape, dropout=0.2):
    """
    ...
    return keras.Model(inputs, outputs, name="MobileNetV3Small_48x300")
...
def main():
    ...
    print(f"\nAblation 2d: MobileNetV3Small @ 48×300 (NATIVE)")
    ...
    # Create model
    print(f"\n{'=' * 70}")
    print("CREATING MOBILENETV3SMALL MODEL (48×300 NATIVE)")
    print(f"{'=' * 70}")
    model = create_mobilenetv3_small_48x300(num_classes, config['input_shape'], config['dropout'])
    model.summary()
    ...
    print("FINAL RESULTS")
    print(f"{'=' * 70}")
    print(f"Model:                   2d_mobilenetv3 (48×300 NATIVE)")
    print(f"Augmentation:            {config['augmentation_mode']}")
    ...
    print(f"Expected: Native 48×300 should preserve resolution better")
    print(f"{'=' * 70}")
